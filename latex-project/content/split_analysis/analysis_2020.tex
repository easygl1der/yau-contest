...
\begin{itemize}
    ...
    \item \textbf{Uniformization Theorem:} We use the standard theorem that a compact Riemann surface of genus at least $2$ has the unit disk $\mathbb{D}$ as universal cover. This is a heavy but natural dependency for this problem; the rest of the proof is then a one-line Liouville argument after lifting.
...
\begin{enumerate}
    ...
    This is a smooth Fermat curve. Indeed, if all three partial derivatives $2020z_j^{2019}$ vanished at a projective point on the curve, then $z_1=z_2=z_3=0$, which is not a point of projective space. \\[2ex]
    ...
    \item \textbf{Lift to the Universal Cover:} By the Uniformization Theorem, the universal cover of $\mathcal{C}$ is the unit disk $\mathbb{D}$. Let $\pi:\mathbb{D}\to\mathcal{C}$ be the covering map. Since $\mathbb{C}$ is simply connected, the covering-space lifting property gives a holomorphic lift $\widehat{\varphi}: \mathbb{C} \to \mathbb{D}$ such that $\pi \circ \widehat{\varphi} = \varphi$. \\[2ex]
