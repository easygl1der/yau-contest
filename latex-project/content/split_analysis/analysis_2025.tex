...
There is a statement risk in the printed formulation.  The integral defining $d(\gamma)$ only makes sense for curves whose image avoids the origin.  Moreover, equality of this winding number classifies ordinary homotopy classes of maps $\mathbb{S}^1\to\mathbb{C}^*$, but it does not by itself classify regular homotopy classes of immersed plane curves.

For example, with $0<\varepsilon<1$ and $2\varepsilon>1$, set
\[
\gamma_0(s)=e^{2\pi is},
\qquad
\gamma_1(s)=e^{2\pi is}+\varepsilon e^{4\pi is}.
\]
Then $\gamma_1(s)=e^{2\pi is}(1+\varepsilon e^{2\pi is})$ never vanishes, so $d(\gamma_1)=1=d(\gamma_0)$.  Also
\[
\gamma_1'(s)=2\pi i e^{2\pi is}(1+2\varepsilon e^{2\pi is})
\]
never vanishes.  However the tangent direction of $\gamma_0$ winds once, while the tangent direction of $\gamma_1$ winds twice, because the circle $1+2\varepsilon e^{2\pi is}$ winds once around $0$.  The tangent winding number is invariant under regular homotopy.  Hence these two curves have equal $d(\gamma)$ but are not regularly homotopic as immersed plane curves.

The correct replacement is:
\begin{itemize}
\item equal $d(\gamma)$ classifies ordinary homotopy of curves in $\mathbb{C}^*$;
\item regular homotopy of immersed plane curves requires the tangent winding number as the Whitney--Graustein invariant; for immersed curves constrained to avoid the origin, one must also keep the ordinary homotopy class in $\mathbb{C}^*$.
\end{itemize}

Under the weaker ordinary-homotopy interpretation in $\mathbb{C}^*$, the intended argument is as follows.

First, suppose $\gamma_0$ and $\gamma_1$ are homotopic through curves in $\mathbb{C}^*$.  This means there is a continuous deformation $\gamma_t(s)$ between them such that every $\gamma_t$ avoids the origin. Since $d(\gamma_t)$ is an integer for all $t \in [0, 1]$, and it depends continuously on $t$ (because the integral of a smooth family of curves is continuous), the function $f(t) = d(\gamma_t)$ is a continuous function from $[0, 1]$ to $\mathbb{Z}$. The only continuous functions from an interval to the discrete set of integers are constant functions. Therefore, $d(\gamma_0) = d(\gamma_1)$.

Conversely, suppose $d(\gamma_0) = d(\gamma_1) = n$. This means both curves wind around the origin $n$ times.
In algebraic topology, maps from the circle $\mathbb{S}^1$ to the punctured plane $\mathbb{R}^2 \setminus \{0\} \simeq \mathbb{C}^*$ are completely classified by their winding number (the fundamental group $\pi_1(\mathbb{C}^*) \cong \mathbb{Z}$). Because they have the same winding number, they are homotopic in $\mathbb{C}^*$.  This proves the corrected ordinary-homotopy statement.  To assert regular homotopy, one must additionally match the tangent winding number.
\end{solution}
