...
\textbf{Steps:}
\begin{enumerate}
    \item For a compact self-adjoint operator, the nonzero eigenvalues are real and have finite multiplicity.

    \item Eigenspaces corresponding to different eigenvalues are orthogonal because $\mathbf{A}$ is self-adjoint.

    \item Let $\{\lambda_n\}$ be the sequence of nonzero eigenvalues and $\{\varphi_n\}$ the corresponding orthonormal eigenvectors. The closure of the span of these eigenvectors is exactly $(\operatorname{Ker} \mathbf{A})^\perp$.

    \item Any vector $\xi \in \mathbf{H}$ can be uniquely decomposed as $\xi = \xi_{perp} + \xi'$, where $\xi' \in \operatorname{Ker} \mathbf{A}$ and $\xi_{perp} \in \overline{\operatorname{span}\{\varphi_n\}}$.

    \item Thus $\xi = \sum c_n \varphi_n + \xi'$, where $c_n = \langle \xi, \varphi_n \rangle$.

    \item Applying $\mathbf{A}$, we get $\mathbf{A}\xi = \sum c_n \mathbf{A}\varphi_n + \mathbf{A}\xi' = \sum \lambda_n c_n \varphi_n$.
...
\textbf{Steps:}
\begin{enumerate}
    \item \textbf{Continuity:} For any $x$, convexity implies that the slope of secant lines is non-decreasing. This bound on slopes ensures the function is locally Lipschitz and thus continuous.

    \item \textbf{Legendre Transform:} $g(y) = \sup_x (xy - f(x))$. Since $xy-f(x)$ is linear in $y$, $g(y)$ is a supremum of convex (linear) functions, hence $g(y)$ is convex.

    \item \textbf{Differentiability:} A convex function has non-decreasing left and right derivatives $f'_-(x) \leq f'_+(x)$. These are monotonic functions, which can have at most countably many jump discontinuities. $f$ is differentiable exactly where $f'_-(x) = f'_+(x)$.
