...
\begin{enumerate}
    ...
    \item \textbf{Applying the Identity Theorem:} Define a new function $g(z) = f(z) - z$. We have shown that $g(z) = 0$ for all $z$ in the open set $V$. Since $V$ is a non-empty open set, the Identity Theorem forces $g(z) = 0$ for all $z$ in the connected component of $U$ containing $V$.

    \item \textbf{Conclusion:} If $U$ is connected, then $f(z) = z$ for all $z \in U$. Even if $U$ is not connected, the fact that $f$ maps $U$ into $U$ and is non-constant allows us to extend this identity property to the entire domain.
\end{enumerate}
...
\textbf{Steps:}
\begin{enumerate}
    \item \textbf{Define a Detection Function:} We want to find a starting point $x$ and a gap $d > 0$ such that $x, x+d, x+2d, \dots, x+2021d$ are all in $X$. We can track this using an integral: $f(d) = \int 1_X(x) 1_X(x+d) \dots 1_X(x+2021d) dx$. If $f(d) > 0$, then there must be at least one $x$ where all those terms are $1$.

    \item \textbf{Examine the Small Gap Limit:} We look at what happens when the gap $d$ becomes very small. As $d \to 0$, each shifted indicator function $1_X(x+jd)$ looks more and more like the original $1_X(x)$.

    \item \textbf{Continuity Argument:} Due to the continuity of translations in $L^p$, the integral $f(d)$ converges to $\int (1_X(x))^{2022} dx$ as $d \to 0$. Since $1_X$ only takes values $0$ or $1$, $(1_X)^{2022}$ is simply $1_X$.

    \item \textbf{Positive Measure Implies Existence:} The limit of $f(d)$ as $d \to 0$ is $\int 1_X(x) dx$, which is the measure of $X$. Since we are given that this measure is positive, $f(d)$ must be positive for all sufficiently small values of $d$. This guarantees that a gap $d$ and a starting point $x$ exist.
\end{enumerate}
...
\textbf{Steps:}
\begin{enumerate}
    \item \textbf{Expand the Function:} In three-dimensional space, any harmonic function that vanishes at infinity can be written as a series of "spherical harmonics" divided by powers of the distance $|x|$. This is known as a multipole expansion: $u(x) = \frac{a_{00}}{|x|} Y_{00} + \frac{a_{1m}}{|x|^2} Y_{1m} + \dots$

    \item \textbf{Isolate the Leading Term:} We are interested in the behavior of $|x|u(x)$. Multiplying the series by $|x|$, we get: $|x|u(x) = a_{00} Y_{00} + \frac{a_{1m}}{|x|} Y_{1m} + \frac{a_{2m}}{|x|^2} Y_{2m} + \dots$

    \item \textbf{Take the Limit:} As $|x|$ becomes very large (goes to infinity), all the terms like $1/|x|$, $1/|x|^2$, etc., will shrink to zero. The only term left standing is the first one, $a_{00} Y_{00}$.

    \item \textbf{Verify Convergence:} Since $a_{00}$ is a constant and $Y_{00}$ is a constant (the simplest spherical harmonic), the result of the limit is just a fixed number. This proves that the limit exists.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Linear Growth:} Because the derivative is bounded, the function $f(x,y)$ can grow at most linearly with respect to $y$ (i.e., $|f(x,y)| \leq A + B|y|$).

    \item \textbf{Preventing Blow-up:} Solutions to ODEs only stop existing if they "blow up" to infinity in a finite amount of time. However, a function whose speed is limited by linear growth cannot reach infinity in finite time. Thus, the solution exists for all $x$.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Monotonicity:} Because solutions to these types of ODEs cannot cross each other, the map $P$ is "monotonic" (if you start higher, you end higher).

    \item \textbf{Convergence to a Fixed Point:} If we have a bounded solution, the sequence of points $y(0), y(1), y(2), \dots$ is a bounded, monotonic sequence. From calculus, we know every bounded monotonic sequence must converge to a limit. This limit point $y^*$ is a "fixed point" of the map $P$, meaning $P(y^*) = y^*$.

    \item \textbf{Conclusion:} A fixed point of the Poincaré map corresponds exactly to a periodic solution of the original ODE.
\end{enumerate}
