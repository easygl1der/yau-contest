...
\begin{enumerate}
    \item $\int_0^1 |f|^p dm \geq \int_{\{|f|>\lambda\}} |f|^p dm \geq \lambda^p m(\{|f|>\lambda\})$.
    \item Let $g = |f|^p$. Then $g \in L^1$. We want to show $\lambda^p m(\{g > \lambda^p\}) \to 0$.
    \item Let $E_\lambda = \{|f| > \lambda\}$. As $\lambda \to \infty$, $E_\lambda \downarrow \emptyset$ a.e.
    \item By the Dominated Convergence Theorem, $\int_{E_\lambda} |f|^p dm \to 0$ because $f \in L^p$ and the measure of $E_\lambda$ goes to $0$.
    \item Since $\lambda^p m(E_\lambda) \leq \int_{E_\lambda} |f|^p dm$, the limit is 0.
\end{enumerate}
\end{solution}
...
\begin{enumerate}
    \item Continuous functions $C[0,2\pi]$ are dense in $L^2[0,2\pi]$.
    \item For any $f \in C[0,2\pi]$, we can approximate it by periodic continuous functions by slightly modifying it near the boundaries.
    \item By the Stone-Weierstrass Theorem, the span of $\{1, \cos nx, \sin nx\}$ is dense in $C_{per}[0,2\pi]$ under the sup-norm.
    \item Since $\| \cdot \|_2 \leq \sqrt{2\pi} \| \cdot \|_\infty$, density in sup-norm implies density in $L^2$.
\end{enumerate}
\end{solution}
...
\begin{enumerate}
    \item \textbf{Compactness:} Let $\|f_n\|_2 \leq 1$. Then $|Kf_n(s)| \leq 1$ and $|Kf_n(s_1) - Kf_n(s_2)| \leq \sqrt{|s_1-s_2|}$. By Arzel\`a-Ascoli, $\{Kf_n\}$ is precompact in $C[0,1]$, and thus in $L^2[0,1]$.
    \item \textbf{Eigenvalues:} $Kf = \lambda f$.
\end{enumerate}
