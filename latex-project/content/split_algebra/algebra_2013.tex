...
\label{algebra:2013:1}\begin{enumerate}
...
\begin{enumerate}
    \item $n_{13} \equiv 1 \pmod{13}$
    \item $n_{13}$ divides $2$ (the other prime factor of the group's order)
\end{enumerate}
...
\label{algebra:2013:2}Consider $f \in \mathbb{Z}_{>0}$ and nonzero vector spaces $V_i$ indexed by $i \in \mathbb{Z}/f\mathbb{Z}$. Suppose that there are linear maps $\phi_i: V_i \to V_{i+1}$ and $\psi_i: V_i \to V_{i-1}$ such that
...
Define $\ell_1 = \text{span}\{v_1\}$. Inductively, after choosing a nonzero vector $v_i$ spanning $\ell_i$, set $\ell_{i+1}=\text{span}\{\phi_i(v_i)\}$ when $\phi_i(v_i)\ne 0$, and otherwise choose any line in $V_{i+1}$.
...
\label{algebra:2013:3}For a parameter $t = (t_0, t_1, \dots, t_5) \in \mathbb{F}_5^6$ with $t_0 \neq 0$ and $\{t_i, i > 0\}$ an ordering of elements in $\mathbb{F}_5$, define a polynomial
...
\label{algebra:2013:4}For $k$ a non-negative integer, let $V_k := \mathbb{R}[x]_{\leq k}$ be the vector space of real polynomials of degree at most $k$ with an action by $SL_2(\mathbb{R})$.
\end{exercise}
...
\label{algebra:2013:5}Let $A$ be an $n \times n$ skew symmetric real matrix.
\begin{enumerate}
...
\begin{itemize}
    \item \textbf{Eigenvalues:} Let $\lambda$ be an eigenvalue with eigenvector $v$. Then $Av = \lambda v$.
    ...
    \item \textbf{Orthogonality:} $(e^A)^T = e^{A^T} = e^{-A}$.
...
\label{algebra:2013:6}Let $SL_2(\mathbb{F}_p)$ be the group of $2 \times 2$ matrices over $\mathbb{F}_p$ with determinant 1.
\end{exercise}
...
\begin{itemize}
    \item \textbf{Case 1: Irreducible over $\mathbb{F}_p$.} The roots lie in $\mathbb{F}_{p^2}$. The roots satisfy $\lambda^{p+1} = 1$ in the multiplicative group. Thus the order divides $p+1$.
    \item \textbf{Case 2: Reducible with distinct roots.} The roots are in $\mathbb{F}_p$. Since their product is 1, the order divides $p-1$.
    \item \textbf{Case 3: Repeated root.} The root must be $1$ or $-1$ (since $\lambda^2=1$).
...
\label{algebra:2013:7}Irreducible complex representations of $S_4$.
\end{exercise}
...
\begin{enumerate}
    \item \textbf{Trivial ($d=1$):} Maps every permutation to 1.
    \item \textbf{Sign ($d=1$):} Maps even permutations to 1 and odd permutations to -1.
    \item \textbf{2D Irrep:} $S_4$ has a normal subgroup $V_4$. $S_4/V_4 \cong S_3$. The 2D irrep of $S_3$ lifts to $S_4$.
    \item \textbf{Standard ($d=3$):} $S_4$ acts on $\mathbb{C}^4$ by permuting coordinates. The subspace where $\sum z_i = 0$ is a 3D irrep.
    \item \textbf{Vieri ($d=3$):} This is the tensor product of the Standard representation and the Sign representation.
\end{enumerate}
\end{solution}
...
\label{algebra:2013:8}Let $F$ be the splitting field of $x^4 - 2$.
\end{exercise}
...
\begin{itemize}
    \item The group $D_8$ itself corresponds to $\mathbb{Q}$.
    \item The trivial group corresponds to $F$.
    \item Subgroups of order 2 correspond to fields of degree 4, such as $\mathbb{Q}(\sqrt[4]{2})$ and $\mathbb{Q}(i\sqrt[4]{2})$.
    \item Subgroups of order 4 correspond to quadratic fields like $\mathbb{Q}(i)$ and $\mathbb{Q}(\sqrt{2})$.
\end{itemize}
\end{solution}
