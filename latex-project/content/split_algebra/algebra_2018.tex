...
- \textbf{For the first row ($i=1$):} $v_p(a_{1j}) \geq \max\{1, n+1-j\}$. As $j$ goes from 1 to $n$, the value $n+1-j$ goes from $n$ down to 1. So, $v_p(a_{11}) \geq n, v_p(a_{12}) \geq n-1, \dots, v_p(a_{1n}) \geq 1$.
- \textbf{For the last row ($i=n$):} $v_p(a_{nj}) \geq \max\{n, n+1-j\}$. Since $n \geq n+1-j$ for all $j \geq 1$, the bound is simply $v_p(a_{nj}) \geq n$ for the entire row.
- \textbf{For the first column ($j=1$):} $v_p(a_{i1}) \geq \max\{i, n+1-1\} = \max\{i, n\}$. Since $i \leq n$, this means $v_p(a_{i1}) \geq n$ for the entire column.
- \textbf{For the last column ($j=n$):} $v_p(a_{in}) \geq \max\{i, n+1-n\} = \max\{i, 1\} = i$.
...
\begin{itemize}
    ...
    \item \textbf{Kummer's Theorem / Carry Bound:} The full theorem says $v_p(\binom{n}{m})$ is the number of carries when adding $m$ and $n-m$ in base $p$. For this problem we only need the weaker bound proved below from Legendre's formula.
...
We prove the needed Kummer-type estimate directly. By Legendre's formula,
\[
v_p\left(\binom{n}{m}\right)
=\sum_{r\geq 1}\left(
\left\lfloor\frac{n}{p^r}\right\rfloor
-\left\lfloor\frac{m}{p^r}\right\rfloor
-\left\lfloor\frac{n-m}{p^r}\right\rfloor
\right).
\]
For each $r$, the summand is either $0$ or $1$, because for real numbers $u,v$,
\[
\lfloor u+v\rfloor-\lfloor u\rfloor-\lfloor v\rfloor\in\{0,1\}.
\]
The summand is zero whenever $p^r>n$. Hence there are at most $\lfloor \log_p n\rfloor$ nonzero summands, and therefore
\[
v_p\left(\binom{n}{m}\right) \leq \lfloor \log_p n\rfloor \leq \log_p n.
\]
