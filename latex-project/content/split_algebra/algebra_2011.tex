...
\begin{exercise}
Let $F \subset \mathbb{R}$ be the set of roots of monic polynomials in $\mathbb{Q}[x]$.
(1) $F$ is a field. (2) Only automorphism is ID.
\end{exercise}
...
\begin{exercise}
Minimal polynomial $m_T$ is irreducible, and $V$ is cyclic. Show $V$ has no proper invariant subspace.
\end{exercise}
...
\subsection*{Team}
\begin{exercise}
$\gcd(f, g) = 1$ in $F[x, y] \implies$ finite intersection.
\end{exercise}
...
\begin{exercise}
$\mathbb{Z}[x, y]/(x^2 - y^n) \cong \mathbb{Z}[x, y]/(x^2 - y^m) \iff n=m$.
\end{exercise}
...
\begin{exercise}
Harmonic sum $H_n = 1 + \dots + 1/n$ is never an integer for $n > 1$.
\end{exercise}
...
\begin{exercise}
$GL_3(\mathbb{F}_7)$. (a) $P_7$, (b) $N(P_7)$, (c) $P_2$.
\end{exercise}
