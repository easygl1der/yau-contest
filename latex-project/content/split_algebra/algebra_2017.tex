...
(a) Show that for every integer $k \geq 0$, $(p^{-k}\mathbb{Z}_p)/\mathbb{Z}_p \cong \mathbb{Z}/p^k\mathbb{Z}$ as abelian groups.

(b) Determine the endomorphism ring of the abelian group $(p^{-k}\mathbb{Z}_p)/\mathbb{Z}_p$ ($k \geq 0$).

(c) Determine the endomorphism ring of the abelian group $\mathbb{Q}_p/\mathbb{Z}_p$.

(d) Determine the endomorphism ring of the abelian group $\mathbb{Q}/\mathbb{Z}$.
\end{exercise}
\begin{solution}
...
(d) The group $\mathbb{Q}/\mathbb{Z}$ can be decomposed into its $p$-primary components:
...
(a) Show that there is a subgroup $A_0$ of $A$ such that $\phi$ restricts to an automorphism of $A_0$ and $A = A_0 \oplus A_{\mathrm{nil}}$.

(b) Show that such a subgroup is unique.
\end{exercise}
\begin{solution}
\textbf{Motivation and Strategy:}
...
(a) Consider the two chains of subgroups:
...
(b) Suppose there exists another subgroup $B$ such that $A = B \oplus A_{\mathrm{nil}}$ and $\phi|_B$ is an automorphism.
...
(a) Show that the set $\mathcal{P}$ of subextensions of $L/F$ not containing $x$ has a maximal element $E$. Let $K/E$ be a nontrivial finite extension contained in $L$. Show that $x \in K$.

(b) Let $K'$ be the Galois closure of $K/E$ in $L$. Show that there exists $g \in G = \mathrm{Gal}(K'/E)$ such that $gx \neq x$.

(c) Deduce that $K/E$ is a cyclic Galois extension.
\end{exercise}
\begin{solution}
\textbf{Motivation and Strategy:}
...
(a) Let $\mathcal{P} = \{ E' \mid F \subseteq E' \subseteq L, x \notin E' \}$.
...
(b) Since $L/F$ is Galois, $L/E$ is also Galois. Let $K'$ be the Galois closure of $K/E$ in $L$. Then $K'/E$ is a finite Galois extension.
...
(c) Every nontrivial subextension of $L/E$ contains $x$ (by part a). Thus $E(x)$ is the \textbf{unique minimal subextension} of $L/E$.
...
(a) Let $0 \leq a < q - 1$ be an integer. Show that $S(X^a) := \sum_{x \in \mathbb{F}_q} x^a$ equals 0. Here we adopt the convention $x^0 = 1$ in $\mathbb{F}_q$ even for $x = 0$.
...
(a) If $a = 0$, $S(X^0) = \sum_{x \in \mathbb{F}_q} 1 = q \cdot 1 = 0$ in $\mathbb{F}_q$ (since $\mathrm{char}(\mathbb{F}_q) = p$).
...
(b) Consider a monomial $X_1^{a_1} \dots X_n^{a_n}$. Its sum over $\mathbb{F}_q^n$ is:
...
(b) For a matrix $A$, we define $\mathrm{N}(A) := \mathrm{tr}(A^*A)$, where $A^* = \bar{A}^T$ is the conjugate transpose of $A$. Assume that $U$ and $V$ are unitary. Deduce that $\mathrm{N}(1 + V) \geq 4$.
\end{exercise}
\begin{solution}
\textbf{Motivation and Strategy:}
...
(a) Let $W = VUV^{-1}$. Since $U$ is diagonalizable and $W$ is a conjugate of $U$, $W$ is also diagonalizable. Since $U$ and $W$ commute, they are simultaneously diagonalizable. Thus, $\mathbb{C}^n$ is the direct sum of the joint eigenspaces $E_{\lambda, \mu}$.
...
(b) For any matrix $A$, $N(A)$ is the squared Frobenius norm. For a unitary matrix $V$:
