...
(a) Show that $\Phi_p = \sum_{i=0}^{p-1} X^i$ is the minimal polynomial of $\zeta_p$ over $\mathbb{Q}$.

(b) Compute the trace $\mathrm{Tr}_{K/\mathbb{Q}}(1 - \zeta_p)$ and the norm $\mathcal{N}_{K/\mathbb{Q}}(1 - \zeta_p)$.

(c) Show that $(1 - \zeta_p)\mathcal{O}_K \cap \mathbb{Z} = p\mathbb{Z}$ and deduce that for all $y \in \mathcal{O}_K$, we have
