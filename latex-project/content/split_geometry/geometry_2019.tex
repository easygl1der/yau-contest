...
\textbf{Steps for (b):}
\begin{enumerate}
    \item \textbf{Simplify variables:} Let $a = z_i - z_j$ and $b = z_j - z_k$. Then $z_k-z_i=-(a+b)$, so
    \[
        \omega_{ij}=\frac{da}{a},\qquad
        \omega_{jk}=\frac{db}{b},\qquad
        \omega_{ki}=\frac{d(z_k-z_i)}{z_k-z_i}
        =
        \frac{da+db}{a+b}.
    \]
    ...
    \begin{aligned}
        ...
        +
        \frac{db}{b}\wedge\frac{da+db}{a+b}
        +
        \frac{da+db}{a+b}\wedge\frac{da}{a}.
    \end{aligned}
    \]
    ...
    \begin{aligned}
        &\frac{da\wedge db}{ab}
        +
        \frac{db\wedge da}{b(a+b)}
        +
        \frac{db\wedge da}{a(a+b)} \\
        &=
        \left(
            \frac{1}{ab}
            -
    ...
    \end{aligned}
    \]
    The coefficient has common denominator $ab(a+b)$ and numerator
    \[
        (a+b)-a-b=0.
    \]
    Hence the whole 2-form is zero.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Determine the sign of $E$:} Integrating the equation over $S^2$ gives
    \[
        E\int_{S^2} f^2\,dA=\int_{S^2}1\,dA=4\pi,
    \]
    because $\int_{S^2}\Delta\ln f\,dA=0$. Since $f>0$, a solution can exist only
    when $E>0$.
    \item \textbf{Use the sharp form of the uniformization input:} If $E>0$, then
    $\widehat g:=E f^2 g$ has Gaussian curvature $1$. A curvature-one metric on
    $S^2$ in the standard conformal class is the pullback of the round metric by a
    M\"obius transformation. Thus $\sqrt{E}f$ is exactly the conformal factor of a
    M\"obius transformation of the round sphere.
    \item \textbf{Write the solutions explicitly:} Identify $S^2$ with the unit
    sphere in $\mathbb{R}^3$ and write $x=(x_1,x_2,x_3)$. The conformal factors of
    M\"obius transformations are
    \[
        \lambda_a(x)=\frac{\sqrt{1-|a|^2}}{1+a\cdot x},
        \qquad |a|<1,
    \]
    after composing with a rotation. Hence all positive solutions are
    \[
        f(x)=\frac{1}{\sqrt{E}}\,
        \frac{\sqrt{1-|a|^2}}{1+a\cdot x},
        \qquad E>0,\quad |a|<1.
    \]
    Equivalently,
    \[
        f(x)=\frac{\sqrt{A^2-|b|^2}}{\sqrt{E}\,(A+b\cdot x)},
        \qquad A>|b|.
    \]
    A direct check uses $\Delta x_i=-2x_i$ on the unit sphere and verifies
    $\Delta\ln f+Ef^2=1$. If $E\leq 0$, the integral identity above rules out
    positive solutions.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Make the obstruction invariant:} Both total spaces deformation
    retract onto their zero sections, so $H_2(TS^2;\mathbb{Z})$ and
    $H_2(S^2\times\mathbb{R}^2;\mathbb{Z})$ are each generated by the corresponding
    core $S^2$. The self-intersection of this generator is the Euler number of the
    normal bundle of the core. For $TS^2$ the normal bundle of the zero section is
    again $TS^2$, so the Euler number is
    \[
        \langle e(TS^2),[S^2]\rangle=\chi(S^2)=2.
    \]
    For $S^2\times\mathbb{R}^2$, the normal bundle of
    $S^2\times\{0\}$ is trivial, so the Euler number is $0$.

    \item \textbf{Conclusion:} A diffeomorphism would induce an isomorphism on
    $H_2\cong\mathbb{Z}$ and preserve the self-intersection form up to sign. The
    possible values $\pm2$ and $0$ are incompatible. Therefore
    $TS^2$ is not diffeomorphic to $S^2\times\mathbb{R}^2$.
\end{enumerate}
...
\begin{exercise}
\label{geometry:2019:10}
Killing fields on weakly negative manifolds.
\end{exercise}

\begin{solution}
\textbf{Restatement of the curvature hypothesis.}
The original problem defines $(M,g)$ to be \textit{weakly negative} if, for every
$p\in M$ and every nonzero $X\in T_pM$, there exists $Y\in T_pM$ such that
\[
    R(X,Y,Y,X)<0.
\]
Here $M$ is compact.
...
\textbf{Part (a).}
Let $X$ be a Killing vector field and set
\[
    f=\frac12 |X|^2.
\]
...
\]
for all vector fields $Y,Z$. First compute the first derivative of $f$:
\[
    Y(f)=\langle\nabla_YX,X\rangle.
\]
Using the Killing equation with $Z=X$ gives
\[
    \langle\nabla_YX,X\rangle
    =
    -\langle Y,\nabla_XX\rangle.
...
\[
    \operatorname{grad} f=-\nabla_XX.
\]
At a fixed point $p$, choose a local extension of a vector $V\in T_pM$ satisfying
$\nabla_VV=0$ at $p$. Then
\[
    ...
    V\langle\nabla_VX,X\rangle.
\]
Expanding at $p$ gives
\[
    (\operatorname{Hess}f)(V,V)
    =
    \langle\nabla_V\nabla_VX,X\rangle+|\nabla_VX|^2.
\]
It remains to identify the second derivative term. Since $X$ is Killing, its flow
acts by isometries, and the standard infinitesimal curvature identity is
\[
    \nabla_Y\nabla_ZX=R(X,Y)Z
\]
at the point after choosing normal extensions of $Y,Z$. Taking $Y=Z=V$ gives
\[
    \langle\nabla_V\nabla_VX,X\rangle
    =
    \langle R(X,V)V,X\rangle.
\]
With the curvature convention in the problem, this is
\[
    \langle R(X,V)V,X\rangle=-R(V,X,X,V).
\]
Therefore
\[
    (\operatorname{Hess}f)(V,V)
...
\textbf{Part (b).}
Assume $X$ is a Killing field. Since $M$ is compact, $f=\frac12|X|^2$ attains a
maximum at some point $p$. At such a point,
\[
    (\operatorname{Hess}f)_p(V,V)\leq 0
\]
for every $V\in T_pM$.

The formula from Part (a) gives
\[
    0
    \geq
    |\nabla_VX|^2-R(V,X,X,V).
\]
If $X(p)\neq 0$, weak negativity applied to the nonzero vector $X(p)$ gives a
vector $Y\in T_pM$ such that
\[
    R(X(p),Y,Y,X(p))<0.
\]
By the curvature symmetries,
\[
    R(Y,X(p),X(p),Y)=R(X(p),Y,Y,X(p))<0.
\]
Taking $V=Y$ in the Hessian formula at the maximum point gives
\[
    (\operatorname{Hess}f)_p(Y,Y)
    =
    |\nabla_YX|^2-R(Y,X(p),X(p),Y)>0,
\]
because the first term is nonnegative and the second term is strictly positive.
This contradicts the fact that the Hessian at a maximum is nonpositive in every
direction. Therefore $X(p)=0$.

Since $p$ is a maximum point of $|X|^2$ and $|X(p)|^2=0$, we get
$|X|^2\equiv 0$ on $M$. Thus $X\equiv 0$. Hence a compact weakly negative
Riemannian manifold has no nontrivial Killing vector fields.
\end{solution}
