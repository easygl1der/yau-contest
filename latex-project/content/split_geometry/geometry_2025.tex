...
\begin{enumerate}
    \item \textbf{Unitary Groups $U(n)$ and $SU(n)$:} $U(n)$ is the group of $n \times n$ unitary matrices. $SU(n)$ is the subgroup consisting of matrices with determinant 1.
    \item \textbf{Fibration:} There is a fundamental fibration (a special type of mapping) given by $SU(n) \to U(n) \xrightarrow{\det} S^1$. This means $U(n)$ can be thought of as a "bundle" with base $S^1$ and fiber $SU(n)$.
    \item \textbf{Long Exact Sequence of Homotopy Groups:} For any fibration $F \to E \to B$, there is a long exact sequence:
    \[ \dots \to \pi_k(F) \to \pi_k(E) \to \pi_k(B) \to \pi_{k-1}(F) \to \dots \]
    \item \textbf{Known Homotopy Groups:}
    \begin{itemize}
        \item For the circle: $\pi_1(S^1) \cong \mathbb{Z}$, and $\pi_k(S^1) = 0$ for $k \geq 2$.
        \item For simple Lie groups like $SU(n)$: $\pi_1(SU(n)) = 0$ (simply connected) and $\pi_2(SU(n)) = 0$ for $n \geq 1$. Also, $\pi_3(SU(n)) \cong \mathbb{Z}$ for $n \geq 2$.
    \end{itemize}
\end{enumerate}

\textbf{Steps for Part (i):}
\begin{enumerate}
    \item We start with the long exact sequence for the fibration $SU(n) \to U(n) \xrightarrow{\det} S^1$:
    \[ \pi_k(SU(n)) \to \pi_k(U(n)) \to \pi_k(S^1) \to \pi_{k-1}(SU(n)) \]
    \item To find $\pi_2(U(n))$, we set $k=2$:
    \[ \pi_2(SU(n)) \to \pi_2(U(n)) \to \pi_2(S^1) \]
    Since $\pi_2(SU(n)) = 0$ and $\pi_2(S^1) = 0$, we have $0 \to \pi_2(U(n)) \to 0$, which forces $\pi_2(U(n)) = 0$.
    \item To find $\pi_3(U(n))$, we set $k=3$:
    \[ \pi_3(SU(n)) \to \pi_3(U(n)) \to \pi_3(S^1) \]
    Since $\pi_3(S^1) = 0$, we have an isomorphism $\pi_3(U(n)) \cong \pi_3(SU(n))$.
    \item For $n=1$, $SU(1)$ is just the identity, so $\pi_3(U(1)) = 0$. For $n \geq 2$, $\pi_3(SU(n)) \cong \mathbb{Z}$, so $\pi_3(U(n)) \cong \mathbb{Z}$.
\end{enumerate}

\textbf{Steps for Part (ii):}
\begin{enumerate}
    \item We look at the tail end of the long exact sequence at the $\pi_1$ level:
    \[ \pi_1(SU(n)) \to \pi_1(U(n)) \xrightarrow{\det_*} \pi_1(S^1) \to \pi_0(SU(n)) \]
    \item We know that $SU(n)$ is simply connected for $n \geq 1$, so $\pi_1(SU(n)) = 0$.
    \item We also know that $SU(n)$ is connected, so $\pi_0(SU(n)) = 0$.
    \item The sequence becomes: $0 \to \pi_1(U(n)) \xrightarrow{\det_*} \mathbb{Z} \to 0$.
    \item An exact sequence of the form $0 \to A \to B \to 0$ implies that the map between $A$ and $B$ is an isomorphism. Thus, $\det_* : \pi_1(U(n)) \to \pi_1(S^1)$ is an isomorphism.
\end{enumerate}
\textbf{Intuition:}
