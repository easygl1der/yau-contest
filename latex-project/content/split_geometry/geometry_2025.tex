...
\begin{itemize}
    \item \textbf{Tangent Bundle and Parallelizability:} A manifold $M$ is called \textit{parallelizable} if its tangent bundle $TM$ is trivial ($TM \cong M \times \mathbb{R}^k$). This is equivalent to saying there exist $k$ smooth vector fields on $M$ that are linearly independent at every point.
    ...
    \item \textbf{Poincaré-Hopf Theorem (Simplified):} If a compact, connected manifold $M$ admits a non-vanishing vector field, then its Euler characteristic $\chi(M)$ must be zero\footnote{The Poincaré-Hopf index theorem states that for any vector field $V$ with isolated zeros, the sum of the indices at the zeros equals the Euler characteristic: $\sum_{x: V(x)=0} \text{ind}_x(V) = \chi(M)$. If a non-vanishing vector field exists, the sum is empty (zero), hence $\chi(M) = 0$.}. Consequently, if $M$ is parallelizable, $\chi(M)$ must be zero.
\end{itemize}
