...
\begin{itemize}
    \item With probability $p$, the walk moves to $a+1$. From there, the probability of hitting 0 is $h_{a+1}$.
    \item With probability $q$, the walk moves to $a-1$. From there, the probability of hitting 0 is $h_{a-1}$.
\end{itemize}
...
To hit state $0$ starting from state $a$, the nearest-neighbor walk must successively hit $a-1,a-2,\ldots,1,0$.
...
\begin{itemize}
    \item \textbf{Case 1 ($p \leq 1/2$):} The probability of moving left is greater than or equal to moving right (drift is negative or zero). Then $q \geq p$, meaning the ratio $q/p \geq 1$. The smallest root in the valid probability range $[0, 1]$ is $h_1 = 1$. The walk is guaranteed to eventually move left.
    \item \textbf{Case 2 ($p > 1/2$):} The probability of moving right is strictly greater than moving left (drift is positive). Then $q < p$, meaning the ratio $q/p < 1$. The smallest root is $h_1 = q/p$. There is a chance the walk drifts to $+\infty$ and never returns.
\end{itemize}
...
\begin{itemize}
    \item Assignment A: $\mathbf{W} = (1, 0, 0)$. Unit 1 treated.
    \item Assignment B: $\mathbf{W} = (0, 1, 0)$. Unit 2 treated.
    \item Assignment C: $\mathbf{W} = (0, 0, 1)$. Unit 3 treated.
\end{itemize}
...
\begin{itemize}
    \item For Assignment A: Treated mean is $X_1 = 1$. Control mean is $(X_2 + X_3)/2 = (2+3)/2 = 2.5$. Difference is $|1 - 2.5| = 1.5$. (Rejected)
    \item For Assignment B: Treated mean is $X_2 = 2$. Control mean is $(X_1 + X_3)/2 = (1+3)/2 = 2.0$. Difference is $|2 - 2| = 0$. (Accepted)
    \item For Assignment C: Treated mean is $X_3 = 3$. Control mean is $(X_1 + X_2)/2 = (1+2)/2 = 1.5$. Difference is $|3 - 1.5| = 1.5$. (Rejected)
\end{itemize}
...
\begin{itemize}
    \item $Y_i(1) = X_i^2 \implies Y_1(1)=1, Y_2(1)=4, Y_3(1)=9$
    \item $Y_i(0) = 0 \implies Y_1(0)=0, Y_2(0)=0, Y_3(0)=0$
\end{itemize}
