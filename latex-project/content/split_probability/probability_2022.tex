...
\begin{enumerate}
    \item $S_n/n \leq 0$ for all $n \geq 1$. (It never goes positive).
    \item $S_n/n = 0$ for some $n \geq 1$. (It actually achieves 0).
\end{enumerate}
...
\begin{itemize}
    \item If $p \geq q$ (drift is $\ge 0$), $\rho = 1$. The walk is guaranteed to reach any positive integer.
    \item If $p < q$ (drift is negative), $\rho = p/q < 1$.
\end{itemize}
...
Scenario:
\begin{enumerate}
    \item In the first $m$ steps, the walk perfectly hits $S_m = k$ such that it never exceeded $k/m$ along the way. For example, if we need $S_3/3 = 1/3$, the sequence $(X_1, X_2, X_3) = (-1, 1, 1)$ gives sums $(-1, 0, 1)$, and averages $(-1, 0, 1/3)$. The maximum so far is $1/3$. This specific finite prefix has a strictly positive probability ($p^a q^b > 0$).
    \item After step $m$, we need the walk to *stay away* from the line $y = (k/m)n$. We require $S_n/n < k/m$ for all $n > m$.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{The tiny numbers:} If $X < 0.1$, then $X \times 10 < 1$. When we take $(10X)^m$, it goes to zero. This means $10^m X^m$ is a tiny number, so its integer part is always 0. The $m$-th digit will be exactly 0. This "dead zone" takes up $1/10$ of the probability space $[0, 0.1]$.
    \item \textbf{The chaotic numbers:} If $X > 0.1$, then $(10X)^m$ grows huge as $m \to \infty$. It wraps around the modulo 10 operation millions of times. Because the function is incredibly steep, a tiny change in $X$ sweeps through all 10 digits very evenly. For the remaining $9/10$ of the probability space, the digits 0 through 9 are hit with perfectly equal probability ($1/10$ each).
\end{enumerate}
