...
\begin{enumerate}
    \item Terms involving only $x_i$: $\omega_{ii} x_i^2$
    \item Terms involving only $x_j$: $\omega_{jj} x_j^2$
    \item Terms involving both $x_i$ and $x_j$: $\omega_{ij} x_i x_j + \omega_{ji} x_j x_i = 2\omega_{ij} x_i x_j$ (since $\Omega$ is symmetric).
    \item Terms involving $x_i$ and some $x_k$ (where $k \neq i, j$): $2x_i \sum_{k \neq i,j} \omega_{ik} x_k$
    \item Terms involving $x_j$ and some $x_k$ (where $k \neq i, j$): $2x_j \sum_{k \neq i,j} \omega_{jk} x_k$
    \item Terms that involve neither $x_i$ nor $x_j$: Let's call this sum $C(\mathbf{x}_{-(i,j)})$.
\end{enumerate}
