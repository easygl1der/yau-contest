...
\begin{itemize}
    ...
    \item \textbf{Complete Sufficient Statistics in Regression:} For a normal linear model $\mathbf{Y} \sim N(X\boldsymbol{\beta}, \sigma^2 I)$, the complete sufficient statistics may be taken as the fitted values $\hat{\mathbf{Y}} = X\hat{\boldsymbol{\beta}}$ and the residual sum of squares $S^2 = \|\mathbf{Y} - \hat{\mathbf{Y}}\|^2$. The needed independence here follows directly from the orthogonal normal decomposition, not from an ancillary-completeness argument.
...
Crucially, $P_X\mathbf{Y}$ and $(I_n-P_X)\mathbf{Y}$ are independent normal vectors. Indeed, their covariance is
\[
\operatorname{Cov}(P_X\mathbf{Y},(I_n-P_X)\mathbf{Y})
=\sigma^2P_X(I_n-P_X)=0,
\]
and zero covariance implies independence for jointly normal vectors. Since $S^2$ is a function only of $(I_n-P_X)\mathbf{Y}$, it follows that $X\hat{\boldsymbol{\beta}}=P_X\mathbf{Y}$ and $S^2$ are independent.
...
As the prior variance $\tau^2 \to \infty$,
\[
r(\pi_\tau)=p\frac{\tau^2}{\tau^2+1}\to p.
\]
For any estimator $\delta$,
\[
\sup_{\eta}R(\eta,\delta)
\geq \int R(\eta,\delta)\,\pi_\tau(d\eta)
\geq r(\pi_\tau).
\]
The first inequality says that the worst-case risk dominates the average risk under any prior; the second says that the Bayes rule minimizes that prior-average risk. Taking the supremum over $\tau$ gives
\[
\inf_\delta\sup_\eta R(\eta,\delta)\geq p.
\]
The estimator $\delta(Y)=Y$ has constant risk exactly $p$, so
\[
\inf_\delta\sup_\eta R(\eta,\delta)=p,
\]
and $\delta(Y)=Y$ is minimax. Translating back to the original coordinates, this is exactly the estimator $\mathbf{d}=\mathbf{X}$.
\end{solution}
