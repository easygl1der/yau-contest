...
\begin{exercise}\label{probability:2019:1}
...
\begin{exercise}\label{probability:2019:2}
...
\begin{exercise}\label{probability:2019:3}
...
\begin{exercise}\label{probability:2019:4}
...
\textbf{As stated, this Gaussian claim needs an extra regularity condition.}
Large sample sizes alone do not force a finite-population randomization distribution to
look Gaussian. For example, if one unit has a potential outcome whose centered value is
of the same order as the whole standard deviation, the random assignment of that single
unit leaves a visible two-point component in the distribution, so a central limit theorem
cannot hold.

A standard correct version assumes treatment fractions bounded away from $0$ and $1$,
and a finite-population Lindeberg condition such as
\[
\frac{\max_i |Y_i(1)-\bar Y(1)|^2}{\sum_{j=1}^{N_t+N_c}(Y_j(1)-\bar Y(1))^2}\to 0,
\qquad
\frac{\max_i |Y_i(0)-\bar Y(0)|^2}{\sum_{j=1}^{N_t+N_c}(Y_j(0)-\bar Y(0))^2}\to 0.
\]
Under this condition, Hajek's finite-population central limit theorem gives
\[
\frac{\hat{\tau}-\tau}{\sqrt{\operatorname{Var}(\hat{\tau})}}
\xrightarrow{d} N(0,1).
\]
A small simulation should therefore use non-dominating potential outcomes, for instance
bounded or light-tailed generated potential outcomes, and compare the histogram of
re-randomized values of $\hat\tau$ with the normal approximation using the finite-population
variance above.
...
\begin{exercise}\label{probability:2019:5}
...
\begin{exercise}\label{probability:2019:6}
...
\begin{exercise}\label{probability:2019:7}
...
Imagine building a random permutation of $n$ people by having them sit at circular tables. The 1st person sits at a new table. The 2nd person can either sit with the 1st person or start a new table. In a uniformly random permutation, the probability that the $k$-th person starts a new table is exactly $1/k$.
...
\begin{exercise}\label{probability:2019:8}
