...
\begin{enumerate}
    \item $|X_n|^s \xrightarrow{d} |X|^s$
    \item The sequence $\{|X_n|^s\}$ is Uniformly Integrable.
    \item $\mathbb{E}[|X|^s] < \infty$.
\end{enumerate}
...
\begin{enumerate}
    \item The particle never visits $y$ while the camera is on. The count is 0.
    \item The particle visits $y$ at least once. Let's focus on the very *first* time it hits $y$ while the camera is on. Let's call this time $\tau$.
\end{enumerate}
...
- \textbf{Row 1:} It must be linearly independent from the empty set. This just means it cannot be the zero vector.
There is exactly $1$ zero vector, so there are $2^n - 1$ valid choices.
The probability of a valid 1st row is $\frac{2^n - 1}{2^n}$.
- \textbf{Row 2:} It must be linearly independent from Row 1. This means it cannot be in the span of Row 1.
...
- \textbf{Row $k$:} It must be linearly independent from the first $k-1$ rows. Assuming the first $k-1$ rows are independent, their span is a subspace of dimension $k-1$.
...
The most standard algorithm in clinical trial design to achieve this is the \textbf{Random Allocation Rule}.
