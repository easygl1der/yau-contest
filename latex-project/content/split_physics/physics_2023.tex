...
\textbf{Prerequisites / Core Concepts:}
\begin{itemize}
    \item \textbf{2D Harmonic Oscillator:} The energy levels of a 2D isotropic harmonic oscillator are $E = (n_x + n_y + 1)\hbar\omega$, characterized by quantum numbers $n_x, n_y \ge 0$.
    \item \textbf{Degenerate Perturbation Theory:} When energy levels are shared by multiple states, we must diagonalize the perturbation matrix within that degenerate subspace to find the true energy shifts.
...
\end{equation*}
\begin{itemize}
    \item \textbf{Ground State ($g=1$):} The first-order correction is $\Delta E_0 = \langle 0, 0 | V | 0, 0 \rangle = 0$, since $a_x|0\rangle = 0$ and $\langle 0|a_x^\dagger = 0$.
    \item \textbf{First Excited State ($g=2$):} We form the $2 \times 2$ perturbation matrix in the basis $\{|1, 0\rangle, |0, 1\rangle\}$.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Second Excited State ($g=3$):} We construct the $3 \times 3$ matrix in the basis $\{|2, 0\rangle, |1, 1\rangle, |0, 2\rangle\}$.
\end{itemize}
...
\textbf{Prerequisites / Core Concepts:}
\begin{itemize}
    \item \textbf{Maxwell's Equations:} The fundamental differential equations that describe how electric and magnetic fields are generated and altered by each other and by charges and currents.
    \item \textbf{Electromagnetic Potentials:} Scalar potential $\phi$ and vector potential $\vec{A}$, which are auxiliary fields from which physical fields ($\vec{E}$ and $\vec{B}$) are derived.
    \item \textbf{Gauge Transformation:} A mathematical shift in the potentials that leaves the physical observables ($\vec{E}$ and $\vec{B}$) completely unaffected.
    \item \textbf{Lorentz Gauge:} A specific condition chosen to decouple and simplify the wave equations for the potentials, revealing relativity naturally.
    \item \textbf{Retarded Potentials:} Solutions indicating that changes in potentials propagate outward at the speed of light, meaning the field depends on the history of the source.
\end{itemize}
...
\textbf{Prerequisites / Core Concepts:}
\begin{itemize}
    \item \textbf{Einstein Field Equations:} The equations relating the geometry of spacetime to the energy within it: $R_{\mu\nu} - \frac{1}{2} R g_{\mu\nu} + \Lambda g_{\mu\nu} = 0$.
    \item \textbf{Trace of a Tensor:} Contracting a tensor with the metric, e.g., taking the trace of the field equations by multiplying by $g^{\mu\nu}$. In 4D spacetime, the trace of the metric is $g^{\mu\nu} g_{\mu\nu} = 4$.
    \item \textbf{Spherically Symmetric Ansatz:} A guessed form for a non-rotating, symmetric spacetime metric: $ds^2 = -f(r) dt^2 + f(r)^{-1} dr^2 + r^2 d\Omega^2$.
    \item \textbf{Killing Vector Field:} A vector field that points in the direction of a continuous symmetry of the spacetime metric.
\end{itemize}
...
\textbf{Prerequisites / Core Concepts:}
\begin{itemize}
    \item \textbf{Feynman Rules:} A systematic recipe for translating pictorial Feynman diagrams into mathematical integrals for scattering amplitudes.
    \item \textbf{Self-Energy ($\Pi$):} The momentum-dependent correction to a particle's mass arising from its interactions with its own field (virtual particle emission and reabsorption).
    \item \textbf{Dimensional Regularization:} A mathematical trick to tame infinite integrals by analytically continuing the number of spacetime dimensions to $d = 4 - \epsilon$, turning the infinity into a simple pole $1/\epsilon$.
    \item \textbf{Renormalization:} The procedure of absorbing infinite corrections into unobservable "bare" parameters (like $\delta m^2$) to yield finite, measurable physical predictions.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Propagator:} The inverse of the kinetic operator yields the scalar propagator: $\Delta(p) = \frac{-i}{p^2 + m^2}$.
    \item \textbf{Interaction Vertex:} The interaction term is $-\frac{1}{3!} g \phi^3$. The vertex factor, obtained by removing the fields and the symmetry factor $3!$, is simply $-ig$.
\end{itemize}
