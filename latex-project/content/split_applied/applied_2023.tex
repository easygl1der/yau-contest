...
(c) Take $f ( x ) = \sin x$ and $x _ { 0 } = 0$ . Prove that both $D _ { h } ^ { + } f ( x _ { 0 } )$ and $D _ { h } ^ { 0 } f ( x _ { 0 } )$ converge quadratically to $f ^ { \prime } ( x _ { 0 } )$ as $h \to 0$ and that in fact they produce the same approximation to $f ^ { \prime } ( x _ { 0 } )$ in this particular case.
\end{exercise}
