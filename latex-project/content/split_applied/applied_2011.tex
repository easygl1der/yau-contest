...
As stated, the inequality is true once the expectations are interpreted in the usual
integrable sense, for example $E|X|<\infty$. The tempting Jensen reduction to
$E|X+E X|$ is not sufficient; the following proof uses the iid symmetry directly.

First compare $X+Y$ with $X-Y$. For real numbers $x,y$,
\[
|x+y|-|x-y|=2\,\operatorname{sgn}(xy)\min\{|x|,|y|\},
\]
with the right side understood as $0$ if $xy=0$. Taking expectations and using the
layer-cake representation for the minimum gives
\[
\begin{aligned}
E|X+Y|-E|X-Y|
&=2\int_0^\infty
\Big(P(X>t)^2+P(X<-t)^2\\
&\hspace{35mm}-2P(X>t)P(X<-t)\Big)\,dt\\
&=2\int_0^\infty \big(P(X>t)-P(X<-t)\big)^2\,dt\geq 0.
\end{aligned}
\]
Thus
\[
E|X-Y|\leq E|X+Y|.
\]
On the other hand, the triangle inequality gives, pointwise,
\[
2|X|=|(X+Y)+(X-Y)|\leq |X+Y|+|X-Y|.
\]
Taking expectations and using the previous inequality,
\[
2E|X|\leq E|X+Y|+E|X-Y|\leq 2E|X+Y|.
\]
Therefore
\[
E|X+Y|\geq E|X|.
...
\begin{solution}
\textbf{(a)} Since $\nabla f(x)=Ax-b=-r(x)$, along the line $x_n+\alpha p_n$ we have
\[
\frac{\mathrm{d}}{\mathrm{d}\alpha}f(x_n+\alpha p_n)
=p_n^T(Ax_n-b)+\alpha p_n^TAp_n
=-p_n^Tr_n+\alpha p_n^TAp_n.
\]
Because $A$ is positive definite, $p_n^TAp_n>0$ whenever $p_n\neq0$, so the critical point is the unique minimizer:
\[
\alpha_n=\frac{p_n^Tr_n}{p_n^TAp_n}.
\]
The construction gives $p_n=r_n+\beta_{n-1}p_{n-1}$ and $p_{n-1}^Tr_n=0$, hence $p_n^Tr_n=r_n^Tr_n$, which is the stated formula.

\textbf{(b)} We argue by induction.  The choice of $\beta_n$ gives
\[
p_{n+1}^TAp_n=(r_{n+1}+\beta_np_n)^TAp_n
=r_{n+1}^TAp_n+\beta_np_n^TAp_n=0.
\]
For $i<n$, use $r_{n+1}=r_n-\alpha_nAp_n$ and the induction hypothesis $p_i^Tr_n=0$ and $p_i^TAp_n=0$ to get
\[
p_i^Tr_{n+1}=p_i^Tr_n-\alpha_np_i^TAp_n=0.
\]
Then $p_{n+1}=r_{n+1}+\beta_np_n$ is $A$-orthogonal to all earlier $p_i$: for $i<n$, both $r_{n+1}^TAp_i$ and $p_n^TAp_i$ vanish by the same induction relation applied to the residual recurrence.  Thus the search directions are mutually $A$-conjugate and $p_i^Tr_n=0$ for $i<n$.

\textbf{(c)} Since
\[
p_{j}=r_j+\beta_{j-1}p_{j-1},
\]
each $p_j$ lies in $\operatorname{span}\{r_0,\dots,r_j\}$.  Conversely,
\[
r_j=p_j-\beta_{j-1}p_{j-1},
\]
so each $r_j$ lies in $\operatorname{span}\{p_0,\dots,p_j\}$.  Therefore
\[
\operatorname{span}\{p_0,\dots,p_{n-1}\}
=\operatorname{span}\{r_0,\dots,r_{n-1}\}
=K_n.
\]

\textbf{(d)} Part (b) gives $p_i^Tr_n=0$ for every $i<n$.  Since $K_n$ is spanned by $p_0,\dots,p_{n-1}$, it follows immediately that $r_n$ is orthogonal to $K_n$.
\end{solution}
...
g(x)=E|X-x|.
\]
The function $g$ is convex as an expectation of convex functions.  Its one-sided derivatives are
\[
g'_-(x)=P(X<x)-P(X\geq x),
\qquad
g'_+(x)=P(X\leq x)-P(X>x).
\]
A point $a$ minimizes a convex function if and only if $0$ lies between its left and right derivatives:
\[
g'_-(a)\leq0\leq g'_+(a).
\]
These inequalities are exactly
\[
P(X<a)\leq \frac12,\qquad P(X\leq a)\geq \frac12.
\]
Equivalently,
\[
P(X\geq a)\geq \frac12,\qquad P(X\leq a)\geq \frac12,
\]
which is the usual definition of a median.  Hence $a$ minimizes $E|X-x|$ if and only if $a$ is a median of $X$.
\end{solution}
...
\]
With the improper flat prior $\pi(\theta)\propto1$, the posterior density is
\[
\pi(\theta\mid y)=
\frac{\prod_{i=1}^n f(y_i-\theta)}
{\int_{-\infty}^{\infty}\prod_{i=1}^n f(y_i-t)\,dt},
\]
provided the normalizing integral is finite.

The prior is noninformative in the standard location-family sense because it is invariant under translations: shifting all observations and the parameter by the same constant does not change the prior density or the form of the posterior.  It is the right Haar measure for the translation group.

For the confidence statement, let a credible interval be equivariant:
\[
C(y+c)=C(y)+c.
\]
If
\[
P(\theta\in C(Y)\mid Y=y)=0.95
\]
under the flat-prior posterior, then by the change of variables $z_i=y_i-\theta$ the posterior probability can be rewritten as
\[
\frac{\int I(0\in C(z))\prod_i f(z_i)\,d\theta}
{\int \prod_i f(z_i)\,d\theta}.
\]
The same integral is the frequentist coverage probability $P_\theta(\theta\in C(Y))$, and translation invariance makes it independent of $\theta$.  Thus such a 95\% posterior probability interval is also a 95\% confidence interval.  The symmetry of $f$ justifies the usual centered equal-tail choice, but the key point is location equivariance.
\end{solution}
...
Therefore, in the conditional experiment given $S_1+S_2>0$,
\[
\hat{\theta}=\frac{S_1}{S_1+S_2}
\]
is unbiased.  Unconditionally, defining the ratio to be $0$ when $S_1+S_2=0$ gives expectation
\[
\theta\,P(S_1+S_2>0),
\]
so the usual ratio is not unconditionally unbiased unless the zero-total event is excluded by design.

(b) For the conditional experiment with total count $m>0$, $S_1$ is sufficient for the binomial parameter $\theta$ by the factorization criterion.  It is also complete in this finite-sample binomial model.  Indeed, if
\[
E_\theta h(S_1)=0
\]
for every $0<\theta<1$, then
\[
\sum_{s=0}^m h(s){m\choose s}\theta^s(1-\theta)^{m-s}=0
\]
for every $0<\theta<1$.  Dividing by $(1-\theta)^m$ and putting $u=\theta/(1-\theta)$ gives the polynomial identity
\[
\sum_{s=0}^m h(s){m\choose s}u^s=0
\]
for all $u>0$.  Hence every coefficient is zero, so $h(s)=0$ for all $s=0,\ldots,m$.  Thus $S_1/m$ is the UMVU estimator in the conditional experiment by the Lehmann-Scheffé theorem.  For the original two-Poisson experiment without conditioning, the question needs this qualification because the zero-total event prevents the displayed ratio from being exactly unbiased.

(c) In the conditional binomial model,
\[
\operatorname{Var}\left(\frac{S_1}{m}\mid S_1+S_2=m\right)=\frac{\theta(1-\theta)}{m},
\]
which attains the Cramer-Rao lower bound for estimating $\theta$ from $m$ binomial trials.  In the unconditional two-Poisson model, after the qualification above, the ratio estimator should not be claimed to attain an unconditional Fisher-information bound without separately specifying how the zero-total event is handled.
\end{solution}
