...
\begin{exercise}
\label{applied:2013:2}
%(15 pts) Finite time extinction and hyper-contractivity are important properties in modeling of some physical and biology systems. The essence of estimates is given by the following problem for ODE.

Assume $y(t)\geq 0$ is a $C^{1}$ function for $t>0$ satisfying $y'(t)\leq\alpha-\beta y(t)^{a}$ for $\alpha>0$, $\beta>0$, then

\begin{enumerate}
\item[(a)] %(10 pts) For $a>1$, $y(t)$ has the following hyper-contractive property:
\[
