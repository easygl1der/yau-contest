...
1. Given a Euclidean triangle $[v_{i}, v_{j}, v_{k}]$ with edge lengths $l_{i}, l_{j}, l_{k}$ and corner angles $\theta_{i}, \theta_{j}, \theta_{k}$ (see Figure 1), we treat the angles as the functions of edge lengths, namely, $\theta_{i} = \theta_{i}(l_{i}, l_{j}, l_{k})$
...
2. Let $A \in \mathbb{R}^{n \times n}$ be symmetric and let $\| q_{1} \|_{2} = 1$ . Consider the following Lanczos iteration:
