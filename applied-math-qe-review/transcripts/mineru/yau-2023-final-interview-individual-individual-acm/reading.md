...
2. Let $A \in \mathbb{C}^{n \times n}$ be a self-adjoint matrix wth k dominant eigenvalues, which are denoted by $\lambda_{j}, j = 1, 2, \cdots, n$ . In particular, we have
...
4. For the advection equation $u_{t} + au_{x} = 0$ with $a > 0,$ , consider the five-point stencil:
