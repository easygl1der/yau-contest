...
Problem 5. Let $A \in \mathbb{R}^{n \times m}$ with rank $r < \operatorname{min}(m, n)$ . Let $A = U \Sigma V^{T}$ be the SVD of A, with singular values $\sigma_{1} \geq \sigma_{2} \geq \cdot \cdot \cdot \geq \sigma_{r} > 0$
