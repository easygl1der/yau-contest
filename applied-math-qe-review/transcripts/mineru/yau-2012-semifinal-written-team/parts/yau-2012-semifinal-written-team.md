...
$$
| u (x) - p _ {n} (x) | \leq \frac {C}{n ^ {k + 1}} \quad \forall 0 \leq x \leq \frac {1}{n},\tag{1}
$$

where the constant C is independent of $n _ { \mathrm { : } }$ , prove
...
$$
- \frac {d ^ {2}}{d x ^ {2}} u (x) = f (x), \quad 0 <   x <   1,
$$
...
$$
\left| a _ {i i} \right| > \sum_ {j = 1, j \neq i} ^ {N} \left| a _ {i j} \right| \text {   for   all   } 1 \leq i \leq N,
$$
...
$$
0 <   \omega_ {i} <   \frac {2}{1 + l _ {i} + u _ {i}} \text { for   all } 1 \leq i \leq N
$$

4. The famous RSA cryptosystem is based on the assumed dificulty of factoring integers $N = p q$ (called RSA integers) which are products of two large primes $p$ and $q$ which should be kept secret. Currently $p$ and q are chosen to be about 500 bits long, that is,

$$
p, q \approx 2 ^ {5 0 0}.
$$

Assume someone uses the following algorithm to find secret n-bit primes $p$ and $q$ to form an RSA integer $N = p q \mathrm { : }$
...
Hint $\begin{array} { r } { \int _ { 0 } ^ { \infty } h r d r = \int _ { 0 } ^ { \dot { r } _ { * } } h r d r } \end{array}$