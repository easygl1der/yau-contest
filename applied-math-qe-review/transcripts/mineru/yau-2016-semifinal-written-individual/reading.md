...
Problem 3. We say that a vector $\mathbf{u} =(u_{1}, \ldots, u_{n}) \in \mathbb{N}^{n}$ is multiplicatively dependent if there is a non-zero vector $\mathbf{k} =(k_{1}, \ldots, k_{n}) \in \mathbb{Z}^{n}$ for which
...
Problem 4. Consider a symmetric matrix $A_{n \times n}$ , and let $\lambda_{i}$ be a simple eigenvalue of A with
