# S.-T. Yau College Student Mathematics Contests 2016 Applied Math. and Computational Math. Individual (5 problems)
...
Here $\delta ^ { 2 }$ is the central second diference operator, and $\delta _ { 0 }$ is the central first diference operator.
...
$$
\mathrm{S} + \mathrm{E} \xrightarrow [ k _ {2} ]{k _ {1}} \mathrm{C} \xrightarrow {k _ {3}} \mathrm{P} + \mathrm{E}
$$
...
where $\mu$ is an approximation of eigenvalue $\lambda _ { i } , \| x _ { k } \| =$ 1and obtain
...
$$
\left\| x _ {k + 1} - (\pm) q _ {i} \right\| = O \left(\left| \lambda_ {i} - \mu \right| + \epsilon\right).
$$
