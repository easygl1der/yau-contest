...
1. Let $H = \{h_{1}, h_{2}, \cdot \cdot \cdot, h_{n}\}$ be weights. The power voronoi diagram induced by $(P, H)$ is a cell decomposition of $\mathbb{R}^{2}$
...
2. Circle Packing on one Triangle. Suppose we associate each vertex $v_{i}$ with a circle $c_{i}(v_{i}, r_{i})$ centered at $v_{i}$ with radius $r_{i}$ . All three circles are tangent to each other, the inner circle has radius $r,$ let $u_{i} = \log r_{i}.$ , prove
