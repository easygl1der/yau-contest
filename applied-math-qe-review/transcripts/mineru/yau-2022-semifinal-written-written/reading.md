...
Problem 1. Consider $\left\{p_{i}(x) \right\}_{i = 0}^{\infty}$ , a family of orthogonal polynomials associated with the inner product
...
Problem 3. Let $A \in \mathbb{R}^{\mathbf{m} \times \mathbf{m}}$ be a matrix with entries $a_{ij}$ which satisfy
...
Problem 5. Construct a third-order accurate Lax-Wendroff-type method for $u_{t} + au_{x} = 0(a > 0$ is a constant) in the following way:
...
Problem 6. Suppose you have \$60K to invest and there are 3 investment options available. You must invest in multiples of \$10??. If $d_{i}$ dollars are invested in investment ?? then you receive a net value (as the profit) of $r_{i}(d_{i})$ dollars. For $d_{i} > 0$ we have
