...
1. Given a convex function $f : \mathbb{R}^{n} \cdot$ <sup>R</sup> and a scalar $\alpha > 0$ , the proximal operator $\mathbf{prox}_{\alpha, f}$ of $f$ is defined as the mapping from a point $x \in \mathbb{R}^{n}$ to the unique solution of the minimization problem:
