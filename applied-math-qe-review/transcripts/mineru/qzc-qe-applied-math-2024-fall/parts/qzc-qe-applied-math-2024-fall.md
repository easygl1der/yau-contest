...
(1) (20 points) The n-th Chebyshev polynomial is defined by: $T_{n}(x) = \cos(n{\cdot} \operatorname{arccos}(x))$ Answer the following questions related to $T_{n}(x)$
...
(2) (15 points) Let A be a symmetric positive definite matrix. Assume that the conjuage gradient method is applied to solve the linear system $Ax = b$ , where $x^{*}$ is the exact solution.
...
(6) (20 points) Consider the following boundary value problem for $y = y(x)$ on $[0, 1]$ with $0<\epsilon\ll1$
...
(7) (20 points) Let $f:\mathbb{R}^n\to\mathbb{R}$ be convex and define $\partial f(x)$ to be the subgradient set of f at x.
