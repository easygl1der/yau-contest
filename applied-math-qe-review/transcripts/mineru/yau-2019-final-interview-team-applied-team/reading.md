...
Problem 3. Maximal entropy principle. Consider probability distributions on a discrete random variable X taking on possible values of $x_{1}, x_{2}, \ldots, x_{n}$ . Denote the probability $\operatorname{Pr}(X = x_{i}) = p_{i}, i = 1, \dots, n$ and recall that its Shannon entropy S is
