...
1. Assume that the iteration does not terminate. Show that $Q_{k}$ has orthonormal columns, and that they span $K_{k}$
...
1. Re-write the above ODE into its equivalent integral formulation near $t = t_{n}$ with $t = t_{n} + s$ for $s \in \mathbb{R}$ via the variation-of-constant formula.
