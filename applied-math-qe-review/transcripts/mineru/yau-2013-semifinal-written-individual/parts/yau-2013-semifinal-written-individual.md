...
$$
u (x, y, z, t) = v (\frac {x - c t}{\sqrt {c ^ {2} - 1}}, y, z), \quad c \in \mathbb {R} ^ {3}, \quad | c | > 1.
$$
...
3. Consider the speed v of a ball (density ρ, radius $R )$ falling through a viscous fluid (density $\rho _ { f }$ , viscosity $\mu )$ with drag coeficient given by Stokes’ law $\zeta = 6 \pi R \mu \mathrm { . }$

$$
\frac {4}{3} \pi R ^ {3} \rho \frac {d v}{d t} = \frac {4}{3} \pi R ^ {3} (\rho - \rho_ {f}) g - \zeta v, v (0) = v _ {0}
$$
...
$$
V _ {h} = \{v: v | _ {I _ {j}} \in P ^ {k} (I _ {j}) \qquad 1 \leq j \leq N \}
$$

where

$$
I _ {j} = (x _ {j - 1}, x _ {j}), \qquad 1 \leq j \leq N
$$
...
$$
| | u - u _ {h} | | \leq | | u - v | | \quad \forall v \in V _ {h}
$$
...
Establish a relation between ${ \binom { 0 } { 1 } } ^ { n }$ and $F _ { n }$ and use it to design an eficient algorithm that for a given n computes the n-th Fibonacci number $F _ { n }$ . In particular, it must be more eficient than computing $F _ { n }$ in n consecutive steps.
