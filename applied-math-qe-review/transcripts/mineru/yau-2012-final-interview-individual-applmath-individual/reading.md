...
1. Let $f(x)$ defined on $[0, 1]$ be a smooth function with suficiently many derivatives. $x_{i} ~ = ~ ih$ , where $\begin{array}{r}{h \ = \ \frac{1}{N}} \end{array}$ and $i = 0, 1, \cdots, N$ are uniformly distributed points in $[0, 1]$ . What is the highest integer k such that the numerical integration formula
...
2. The classical Euclidean Algorithm to find the greatest common divisor $\operatorname{gcd}(m, n)$ of two positive integers $m < n$ requires only $O(\log n)$ arithmetic operation. However, it uses division with a reminder, which is a rather slow operation. Your task is to design and analyze a division-free algorithm.
