...
Problem 1. Let $\{X_{n}\}_{n \geq 1}$ be a sequence of real valued, nonnegative random variables. Assume that there are constants $C > 0$ and $\lambda > 0$ such that $\mathbb{E} X_{n} \leq Ce^{- \lambda n}, \ \forall n \geq 1$ Prove that
...
Problem 2. Assume that $X_{1}, \dots, X_{n} \sim U[0, 1]$ (uniform distribution) are i.i.d. Denote $\begin{array}{r}{X_{(1)} = \operatorname{min}_{1 \leq k \leq n} X_{k}} \end{array}$ and $X_{(n)} = \operatorname{max}_{1 \leq k \leq n} X_{k}$ . Let $R = X_{(n)} - X_{(1)}$ be the sample range and $V =(X_{(1)} + X_{(n)}) / 2$ be the sample midvalue.
