...
2. Suppose that $X =(X_{1}, \ldots, X_{n})^{\prime}$ is an observation from the n-dimensional multivariate normal distribution $N_{n}(\theta, I)$ with unknown parameter $\theta \in \mathbb{R}^{n}$ , that is, $X_{i}^{,}$ s are independent of each other with $X_{i} \ \sim \N(\theta_{i}, 1)$ for $i = 1, \ldots, n$ . (i). Derive the maximum likelihood estimator (MLE) of $\begin{array}{r}{\| \theta \|^{2} = \sum_{i = 1}^{n} \theta_{i}^{2}} \end{array}$
