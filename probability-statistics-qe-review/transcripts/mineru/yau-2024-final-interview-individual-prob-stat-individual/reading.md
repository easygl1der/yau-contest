...
Problem 1. Let $\{X_{i}\}_{i \ge 0}$ be iid with density function f and distribution function F . Define $N = \operatorname{min} \{n \geq 1 : X_{n} > X_{0}\}$
...
Problem 2. A fair coin is tossed repeatedly and independently, and the outcome is denoted as $X_{1} X_{2} \cdots$ with $X_{i} = H$ (head) or T (tail).
...
Problem 3. Given a filtration $\{{\mathcal F}_{n}\}$ , i.e., ${\mathcal{F}}_{1} \subseteq{\mathcal{F}}_{2} \subseteq \cdot \cdot \cdot \subseteq{\mathcal{F}}$ , we define ${\mathcal{F}}_{\infty} =$ $\sigma(\cup_{n = 1}^{\infty} \mathcal{F}_{n})$
