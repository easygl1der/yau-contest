...
Which student(s) have the highest probability to get this gift $( \mathrm { i . e . , \ w i n ) \ ? }$
...
$$
f (\mathbf {x}) = \frac {1}{(2 \pi) ^ {\frac {n}{2}} d e t (\Sigma) ^ {\frac {1}{2}}} \exp \left\{- \frac {1}{2} \mathbf {x} ^ {\prime} \Sigma^ {- 1} \mathbf {x} \right\}, \mathbf {x} \in R ^ {n},
$$

where Σ is an $n \times n$ positive definite matrix. Let the $( i , j )$ element of $\Omega = \Sigma ^ { - 1 }$ be $\omega _ { i j }$ $( 1 \leq i , j \leq n )$ . For $1 \leq i \neq j \leq n$ , show that if $\omega _ { i j } = 0$ , then $x _ { i }$ and $x _ { j }$ are conditionally independent when the other elements of x are given.

Problem 5. Letx, y be two independent random vectors in $R ^ { n } \ ( n \geq 3 )$ . Assume that $P ( \mathbf { y } = \mathbf { 0 } ) = 0$ and x has a standard multivariate normal distribution, ${ \mathrm { i . e . , x } } \sim N ( 0 , I _ { n } )$ (a) For any nonzero constant vector $\mathbf { a } \in R ^ { n }$ satisfying $| | \mathbf { a } | | = ( \mathbf { a } ^ { \prime } \mathbf { a } ) ^ { 1 / 2 } = 1$ , prove that

$$
\sqrt {n - 1} \frac {\mathbf {a ^ {\prime} x}}{\sqrt {| | \mathbf {x} | | ^ {2} - (\mathbf {a ^ {\prime} x}) ^ {2}}} \sim t _ {n - 1},
$$

here $t _ { n - 1 }$ stands for a t distribution with $n - 1$ degrees of freedom.

(b) The sample correlation coeficient between $\mathbf { x } = ( x _ { 1 } , . . . , x _ { n } ) ^ { \prime }$ and $\mathbf { y } = ( y _ { 1 } , . . . , y _ { n } ) ^ { \prime }$ is defined as
