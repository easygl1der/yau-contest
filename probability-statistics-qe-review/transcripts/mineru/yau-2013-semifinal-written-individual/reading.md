...
Problem 1. Let $(X_{n})$ be a sequence of random variables.
...
Problem 3. Let $\{X_{n}\}$ be a sequence of independent and identically distributed continuous real valued random variables, and regard n as time. Let $A_{n}$ be the following event:
...
Problem 4. Let $X =(X_{1}, \cdots, X_{n})$ be an iid sample from an exponential density with mean θ. Consider testing $H_{0} : \theta = \theta_{0}$ vs. $H_{1} : \theta > \theta_{0}$ . Let $P(X) = \operatorname{your}$ p-value for an appropriate test.
...
Problem 5. Let $X_{1}, X_{2}$ be iid uniform on $\theta -{\textstyle \frac{1}{2}}$ to $\theta +{\textstyle \frac{1}{2}}$
