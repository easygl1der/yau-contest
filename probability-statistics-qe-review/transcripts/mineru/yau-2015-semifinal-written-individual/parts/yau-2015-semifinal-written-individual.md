...
$$
\mathbb {P} (\sum_ {n} X _ {n} ^ {2} = \infty) = 1.
$$

Problem 4. Let $X _ { 1 } , \ldots , X _ { n }$ be a random sample of size n from the exponential distribution with pdf $f ( x ; \theta ) ~ = ~ \theta ^ { - 1 } \exp ( - x / \theta )$ for $x , \theta \ > \ 0$ , zero elsewhere. Let $Y _ { 1 } = \operatorname* { m i n } \{ X _ { 1 } , \ldots , X _ { n } \}$ . Consider an estimator $n Y _ { 1 }$
