...
(a) $0 < E _ { P } [ X _ { 0 } | \mathcal { G } ] < + \infty , P \mathrm { - a . s . ; }$
...
Problem 4. Suppose $X _ { 1 } , \dots , X _ { n } , \dots .$ is a sequence of random numbers drawn from the uniform distribution $U ( 0 , 1 )$ . One observes these numbers sequentially. At time $n _ { \mathrm { : } }$ one keeps a record of $Y _ { n } \ { \overset { d e f } { = } } \ X _ { ( n ) } = \operatorname* { m a x } _ { i = 1 } ^ { n } X _ { i } = \operatorname* { m a x } \{ Y _ { n - 1 } , X _ { n } \}$ and $Z _ { n } \stackrel { d e f } { = } \bar { X } _ { n } =$ $\begin{array} { r } { \sum _ { i = 1 } ^ { n } X _ { i } / n = ( n - 1 ) / n Z _ { n - 1 } + 1 / n X _ { n } } \end{array}$ and discards all previous recordings.
