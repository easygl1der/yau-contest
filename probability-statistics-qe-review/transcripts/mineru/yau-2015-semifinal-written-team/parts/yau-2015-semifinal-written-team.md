...
Problem 2. Assume a sequence of random variables $X _ { n }$ converges in distribution to a random variable X. Let $\{ N _ { t } , t \geq 0 \}$ be a set of positive integer-valued random variables, which is independent of $( X _ { n } )$ and converges in probability to ∞ as $t \to \infty$ . Prove that $X _ { N _ { t } }$ converges in distribution to $X$ as $t \to \infty$
...
$$
V _ {n} = \Bigl \{\frac {T _ {1}}{S _ {n}}, \frac {T _ {2}}{S _ {n}}, \dots , \frac {T _ {n}}{S _ {n}} \Bigr \}.
$$

Problem 4. Suppose that X and Z are jointly normal with mean zero and standard deviation 1. For a strictly monotonic function $f ( \cdot )$ $\operatorname { c o v } ( X , Z ) ~ = ~ 0$ if and only if cov $( X , f ( Z ) ) = 0$ , provided the latter covariance exists. Hint: $Z$ can be expressed as $Z = \rho X + \varepsilon$ where X and ε are independent and $\varepsilon \sim N ( 0 , \sqrt { 1 - \rho ^ { 2 } } )$
...
2. $\| \Delta _ { S ^ { c } } \| _ { 1 } \leq 3 \| \Delta _ { S } \| _ { 1 }$ , where $S = \{ j : \beta _ { i } ^ { * } \neq 0 \}$ is the support of the vector $\beta ^ { * } , S ^ { c }$ is its complement set, $\Delta _ { S }$ is the subvector of $\Delta$ restricted on the set $S _ { i }$ , and $\| \Delta _ { S } \| .$ 1 is its $L _ { \mathrm { 1 } } \mathrm { - n o r m }$