# Spring 2026 Probability and Statistics PhD Qualifying Examination

> [!info] Sources and Transcriptions
> - Official publication page: [Qiuzhen College Spring 2026 Probability and Statistics Qualifying Examination](https://qzc.tsinghua.edu.cn/info/1072/8041.htm)
> - Official source PDF: [qzc-qe-probability-statistics-2026-spring.pdf](../sources/exams/qzc-qe-probability-statistics-2026-spring.pdf)
> - MinerU reading package: [Full transcription](../transcripts/mineru/qzc-qe-probability-statistics-2026-spring/reading.md). The original Markdown, JSON, and image assets are stored in the `parts/` directory.
> - Corresponding solutions: [Problem-by-problem reference solutions](../solutions/08-2026-spring-solutions.md). Mathematical notation in the solutions uses Obsidian dollar-sign delimiters.
...
Let $X =(X_{1}, \ldots, X_{n})^{T}$ be an $\mathbb{R}^n$-valued Gaussian random vector under a probability measure $\mathbb{P}$, with mean vector $\boldsymbol{\mu} \in \mathbb{R}^{n}$ and covariance matrix $V \in \mathbb{R}^{n \times n}$ . Let $\alpha \in \mathbb{R}^{n}$ and define a new probability measure $\mathbb{Q}$ by
...
Let $\{X_{n}\}_{n \in \mathbb{N}}$ be a sequence of independent random variables with distribution:
...
Let $\{B_{t}\}_{t \ge 0}$ be a one-dimensional standard Brownian motion started from the origin. Find and prove the limit (in distribution) of
...
A Galton-Watson branching process is a stochastic process $\{Z_{n}\}_{n \in \mathbb{N} \cup \{0\}}$ which evolves according to the recurrence formula
...
Let $X_{1}, X_{2},..., X_{n}$ be i.i.d. observations with pdf
...
Let $X_{1}, X_2,\ldots,X_n$ be i.i.d. Bernoulli random variables with unknown success probability $0 ~ < ~ \theta ~ < ~ 1$ . Given $0 ~ < ~ p ~ < ~ 1$ , let $\hat{\xi}_{p}$ be the sample quantile based on the n observations, i.e., ${\hat{\xi}}_{p} = \operatorname{inf} \{x : F_{n}(x) \geq p\}$ , where $F_{n}$ denotes the empirical CDF based on the data.
...
Suppose a single observation $Y | \mu \sim N(\mu, 1)$ , and the prior on µ is $\pi(\mu) = 0.5 \cdot I(\mu =$ $- 1) + 0.5 \cdot I(\mu = 1)$ where $I(\cdot)$ is the indicator function. That is, under this prior, µ is supported on {−1, 1} and takes either value with a probability 0.5. For estimating $\mu,$ let the action space be {−1, 1}.
...
Let $X_{1}, X_{2}, \ldots, X_{n}$ be i.i.d. random variables following $\mathrm{Poisson}(\sqrt{\lambda})$ distribution where $\lambda > 0$
...
Let $Y = \sigma(\rho | U | + \sqrt{1 - \rho^{2}} V)$ where U, V are independent $N(0, 1)$ random variables and $\rho \in(- 1, 1), \sigma \in(0, \infty)$ are unknown parameters.
