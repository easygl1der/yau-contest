...
1.  数学对象是什么，例如假设类、风险、值函数、注意力矩阵；

2.  它在算法里如何表示，例如数组、参数向量、损失函数、优化器状态；

3.  考试会如何考，例如证明界、推导梯度、解释局限、比较模型。

**来源定位：** 结构依据：求真书院 AI 方向博士生资格考试大纲 [@qzc-ai-syllabus-2026]。机器学习理论主要参考 [@shalev2014uml] 与 [@mohri2018foundations]；深度学习参考 [@goodfellow2016deep] 与 [@bishop2023deep]；优化参考 [@nesterov2018convex; @beck2017first]；NLP 参考 [@jurafsky2026slp; @goldberg2017nnnlp]。

##### 陈述层级

本章用 **[定义]**、**[定理]**、**[证明骨架]**、**[启发式解释]** 与 **[工程惯例]** 区分形式结论、证明路线、直觉与工程经验。没有标签的连续正文仅承担叙述和衔接，不应被误读为无条件定理；每个带条件的结论以其所在段落的假设为准。

##### 本章符号约定

向量默认用小写字母，例如 $x,w,z$；矩阵默认用大写字母，例如 $X,Q,K$。概率分布用花体或罗马体，例如 $\mathcal{D}$ 和 $p_\theta$；训练样本统一写为 $S=(z_i)_{i=1}^m$，其中 $z_i=(x_i,y_i)$。经验风险与真实风险分别为 $L_S$ 与 $L_{\mathcal{D}}$。当 $y$ 同时可能表示单个标签和标签向量时，单个标签写作 $y_i$，标签向量写作 $\mathbf{y}$。指示函数统一写作 $\mathbb{I}\{\cdot\}$。

跨模块处采用下表的消歧约定；局部推导沿用其上下文中已经定义的符号，但不得借用同一符号表示表中两个不同对象。

| 对象 | 记号 | 说明 |
| --- | --- | --- |
| 数据分布与样本 | $\mathcal{D}$, $S=(z_i)_{i=1}^m$ | $z_i=(x_i,y_i)$，训练样本量为 $m$ |
| VC 维与输入维数 | $d_{\mathrm{VC}}$, $d_{\mathrm{in}}$ | 不再用同一 $d$ 同时指代二者 |
| 模型维数与网络深度 | $d_{\mathrm{model}}$, $L_{\mathrm{net}}$ | Transformer 宽度与网络层数 |
| 平滑常数与损失 | $L_{\mathrm{sm}}$, $\ell$ 或 $\mathcal{L}$ | $L_{\mathcal{D}}$ 保留给风险，避免混淆 |
...
| 考纲条目 | 本章覆盖位置 | 主要来源 |
| --- | --- | --- |
| 监督学习和 PAC 学习理论 | 监督学习、PAC、可实现 ERM、agnostic PAC、统一收敛 | [@shalev2014uml; @mohri2018foundations] |
| VC 维、Sauer 引理、样本复杂度 | VC 维、增长函数、Sauer 引理、VC 泛化界推导 | [@mohri2018foundations] |
| 模型选择与偏差方差 | approximation/estimation error、bias-variance 分解 | [@shalev2014uml] |
| 线性回归和逻辑回归 | 正规方程、岭回归、逻辑损失的 MLE 与梯度 | [@shalev2014uml] |
| 决策树、最近邻 | 信息增益/Gini、过拟合、$k$-NN 的 bias-variance 与距离尺度 | [@shalev2014uml] |
| SVM 与核方法 | margin、hinge loss、SVM 对偶、kernel trick | [@mohri2018foundations] |
| 压缩感知 | 稀疏测量、$\ell_1$ 松弛、RIP 唯一性 | [@candes2006compressive] |
| 凸集、凸函数、次微分 | 凸一阶条件、次梯度、Fermat rule、sum rule、$\ell_1$ 次微分 | [@nesterov2018convex; @beck2017first] |
| GD、SGD、随机坐标下降 | descent lemma、强凸线性收敛、SGD telescoping、坐标更新 | [@beck2017first; @shalev2014uml] |
| 动量和自适应学习率 | momentum、Nesterov、AdaGrad、Adam 的状态变量与适用场景 | [@nesterov2018convex; @beck2017first] |
| 神经网络框架、训练、逼近和泛化 | MLP、反向传播、逼近/优化/泛化三分法、归一化、残差 | [@goodfellow2016deep; @bishop2023deep] |
| 视觉任务 | 特征提取、恢复、三维重构、光流、识别与分割的输出结构 | [@szeliski2022vision; @bishop2023deep] |
| 无监督学习、隐式正则化、迁移和元学习 | autoencoder、contrastive learning、pretraining、fine-tuning、meta-objective | [@goodfellow2016deep; @bishop2023deep] |
| 视觉生成模型和表达力分析 | VAE、GAN、flow、diffusion 的目标、likelihood、可逆性和采样代价 | [@goodfellow2014gan; @kingma2014vae; @dinh2017realnvp; @ho2020ddpm; @song2021score] |
| 强化学习 | MDP、Bellman 方程、Q-learning、policy gradient、baseline | [@mohri2018foundations; @sutton2018rl] |
| 统计建模和 Word2vec | $n$-gram MLE、skip-gram、negative sampling、GloVe | [@jurafsky2026slp; @goldberg2017nnnlp] |
| Transformer self-attention | $Q,K,V$ 维度、$\sqrt{d_k}$ 缩放、causal mask | [@vaswani2017attention; @jurafsky2026slp] |
| 大模型预训练对齐与 Scaling Law | next-token CE、SFT、RLHF/DPO、幂律与 compute-optimal trade-off | [@kaplan2020scaling; @hoffmann2022training; @rafailov2023dpo] |
| Embedding 知识库推理 | TransE、RAG、MLN、能力边界和失败模式 | [@lewis2020rag; @jurafsky2026slp] |

## 监督学习、PAC、VC 与泛化

**从零教学。** 监督学习的基本问题是：从有限样本中学一个预测规则[^1]，并希望它在未见样本上也表现好。这里的核心困难不是"训练集上能不能拟合"，而是"训练集误差能不能代表总体误差"。

设输入空间为 $\mathcal{X}$，标签空间为 $\mathcal{Y}$，未知数据分布为 $\mathcal{D}$。训练集

$$
S=((x_1,y_1),\ldots,(x_m,y_m))
$$

由 $\mathcal{D}$ 独立同分布抽样得到。假设类 $\mathcal{H}$ 是候选预测器集合，损失函数 $\ell(h,(x,y))$ 衡量预测器 $h$ 在样本 $(x,y)$ 上的错误。经验风险和真实风险分别为
...
这一定义背后的直觉很简单：既然看不到总体分布，就先选训练集上表现最好的模型。但 ERM 是否可靠取决于 $\mathcal{H}$ 的复杂度。如果 $\mathcal{H}$ 太大，它可以把随机噪声也拟合掉，训练误差会系统性偏低。

### PAC 学习

PAC 是 Probably Approximately Correct 的缩写。它把"学会"写成一个概率命题：样本足够多时，学习算法以高概率输出一个误差不超过目标精度的假设。

在二分类可实现情形中，假设存在目标函数 $f\in\mathcal{H}$，标签由 $y=f(x)$ 生成。算法 $A$ PAC 学会 $\mathcal{H}$，意思是对任意精度 $\varepsilon>0$ 和失败概率 $\delta>0$，当样本量
...
这里 $\varepsilon$ 是 accuracy parameter，控制允许多错；$\delta$ 是 confidence parameter，控制允许多大概率失败。样本复杂度 $m_{\mathcal{H}}(\varepsilon,\delta)$ 告诉我们要多少样本才能达到这个保证。
...
固定 $h$ 时，问题就是 Bernoulli 均值集中；若要同时控制所有 $h\in\mathcal{H}$，就要把坏事件

$$
\left|L_S(h)-L_{\mathcal{D}}(h)\right|>\varepsilon
$$

对整个假设类做并集控制。这就是有限假设类界中 $\log|\mathcal{H}|$ 的来源。若 $\mathcal{H}$ 有限，Hoeffding 不等式和 union bound 给出典型形式
...
若 $\mathcal{H}$ 有限，存在某个坏假设仍与训练集完全一致的概率不超过
...
这条界和上面的二侧统一收敛界常数与 $\varepsilon$ 阶不同，原因是它用了更强的"训练误差为零"和"可实现"假设 [@shalev2014uml; @mohri2018foundations]。

**考试提醒。** 考题常要求从"固定 $h$"过渡到"存在 $h$"：第一步写 Bernoulli 误差变量，第二步用集中不等式，第三步对 $\mathcal{H}$ 做 union bound。若漏掉第三步，通常只能得到单个假设的界。

### Agnostic PAC 与统一收敛

现实中目标函数未必属于 $\mathcal{H}$，甚至标签可能含噪声。Agnostic PAC 不假设可实现性，而是要求算法输出接近类内最优的假设：
...
以至少 $1-\delta$ 的概率成立。这里 $\inf_{h\in\mathcal{H}}L_{\mathcal{D}}(h)$ 是*类内最优风险*（class-optimal risk），并不是 approximation error。相对于 Bayes 风险 $L^*_{\mathrm{Bayes}}$ 的近似误差应写作 $\inf_{h\in\mathcal{H}}L_{\mathcal{D}}(h)-L^*_{\mathrm{Bayes}}$。下文若使用近似 ERM，还须把优化误差单列；本段先分析精确 ERM。ERM 的困难变成证明统一收敛：
...
一旦统一收敛成立，ERM 的泛化分析几乎是机械的。令 $h^*\in\operatorname*{arg\,min}_{h\in\mathcal{H}}L_{\mathcal{D}}(h)$，则
...
当 $\mathcal{H}$ 无限时，不能再用 $|\mathcal{H}|$ 直接度量复杂度。VC 理论度量的是假设类在有限点集上实现任意标记的能力。

**[定义]** 给定 $m$ 个点 $x_1,\ldots,x_m$，增长函数定义为
...
如果*存在*一个 $m$ 点集，其所有 $2^m$ 种二值标记都能由 $\mathcal{H}$ 实现，就说该点集被 $\mathcal{H}$ shatter。用上确界而不是最大值，VC 维的严格定义为
...
Sauer 引理说明有限 VC 维会把指数级增长压成多项式增长：若 $\operatorname{VCdim}(\mathcal{H})=d<\infty$，则
...
并约定集合无上界时 $d_{\mathrm{VC}}(\mathcal{H})=\infty$。这条引理的意义是：即使 $\mathcal{H}$ 无限，只要 VC 维有限，在 $m$ 个样本上它能产生的标记模式也只有多项式多；但要得到随机样本上的统一收敛，不能直接把有限类证明中的 $|\mathcal{H}|$ 替换成 $\tau_{\mathcal{H}}(m)$。

##### 从 Sauer 到 VC 泛化界

**[证明骨架]** 以下骨架针对二值类的 $0$--$1$ 损失（更一般地，可替换为有界损失并相应调整常数），并取 $S,S'\overset{\mathrm{iid}}{\sim}\mathcal{D}^m$。令 $S'$ 是独立 ghost sample。先把 $L_{\mathcal{D}}$ 与 $L_{S'}$ 比较，再经 symmetrization 把偏差写成 $S\cup S'$ 上的两份经验风险之差。给定这 $2m$ 个点后，$\mathcal{H}$ 只会产生至多 $\tau_{\mathcal{H}}(2m)$ 个标记模式，最后才对这些模式使用 union bound。因此在常数与满足 symmetrization 前提的样本量范围内，有
...
其中 $c>0$ 是集中不等式给出的常数。若 $\operatorname{VCdim}(\mathcal{H})=d$，Sauer 引理给出
...
这就是 VC 泛化界的来源。考试若要求"说明样本复杂度和 VC 维的关系"，应写出 ghost sample、symmetrization、$\tau_{\mathcal{H}}(2m)$ 与 Sauer 引理这条链，而不是把增长函数直接代入固定假设的 Hoeffding 界。
...
对所有 $h\in\mathcal{H}$ 同时成立。考试中不必执着于常数，但必须看懂 $d/m$ 是主导量：样本量远大于 VC 维时，训练误差才会可靠。
...
引入一个与 $S$ 独立的 ghost sample $S'=(z_1',\ldots,z_m')$。因为 $\mathbb{E}g(z)=\mathbb{E}_{S'}m^{-1}\sum_i g(z_i')$，先把总体期望替换为 ghost sample 平均：
...
这里 $\mathfrak{R}_m(\mathcal{G})=\mathbb{E}_S[\widehat{\mathfrak{R}}_S(\mathcal{G})]$ 是经验 Rademacher complexity 对样本抽样再取期望后的总体版本，$\widehat{\mathbb{E}}_S g=m^{-1}\sum_i g(z_i)$ 是样本平均。

高概率版本再配合 [McDiarmid](https://www.perplexity.ai/search/ebf75353-a7f9-4b19-9be9-febe0e45faac) 或 [Hoeffding 型浓缩不等式](https://www.perplexity.ai/search/434a7a20-826a-4983-be61-18b439a4e602)。这一推导叫 symmetrization，是 Rademacher complexity 真正进入泛化界的地方 [@mohri2018foundations]。


##### Implementation Mapping (实现映射)

在代码里，$S$ 是数据矩阵和标签向量，$h$ 是模型对象，$L_S$ 是训练循环返回的平均 loss，$L_{\mathcal{D}}$ 只能用验证集或测试集近似。理论证明里常说"独立同分布抽样"；工程上对应数据切分、随机种子、去重、避免训练测试泄漏。

##### 考核内容

- 写出 PAC 或 agnostic PAC 的精确定义，区分 $\varepsilon$、$\delta$ 和样本复杂度。

- 从 Bernoulli 指示变量和 union bound 推有限类样本复杂度。

- 定义 shattering、增长函数、VC 维，并用 Sauer 引理得到统一收敛形式。

- 解释 approximation error、estimation error 与模型选择的关系。

- 读懂 Rademacher complexity 的定义，并说明它为什么度量"拟合随机噪声"的能力。

## 模型选择、偏差方差与经典监督模型

**来源定位：** 本节对应考纲"模型选择、偏差方差、线性回归、逻辑回归、决策树、最近邻、SVM、核方法、压缩感知"。基础模型参考 [@shalev2014uml]；核方法参考 [@mohri2018foundations]；压缩感知参考 [@candes2006compressive]。
...
其中 $h^*_{\mathcal{H}}$ 是 $\mathcal{H}$ 内真实风险最小的预测器。模型类越大，approximation error 往往越小，但 estimation error 往往越大。模型类越小，估计更稳定，但可能欠拟合。
...
所以得到 bias squared 加 variance。若观测标签还含噪声 $y=f(x)+\xi$，且 $\mathbb{E}[\xi\mid x]=0$、$\operatorname{Var}(\xi\mid x)=\sigma^2$，预测 $y$ 的均方误差还会多出不可约噪声 $\sigma^2$。
...
把样本写成设计矩阵 $X\in\mathbb{R}^{m\times d}$，其中第 $i$ 行是 $x_i^\top$；标签向量写成 $\mathbf{y}=(y_1,\ldots,y_m)^\top\in\mathbb{R}^m$。忽略偏置或把偏置并入一列常数特征，最小二乘问题为
...
这解释了正则化的数值意义：它把病态矩阵往正定方向推，提高稳定性。 这里 $I\in\mathbb{R}^{d\times d}$ 是单位矩阵，$\lambda\ge0$ 是正则化强度。
...
严格地说，实值核 $k:\mathcal{X}\times\mathcal{X}\to\mathbb{R}$ 必须对称，即 $k(x,x')=k(x',x)$，并对任意有限点集 $x_1,\ldots,x_m$ 以及任意系数 $c\in\mathbb{R}^m$ 满足
...
压缩感知回答一个看似不可能的问题：如果未知信号 $x\in\mathbb{R}^N$ 维度很高，但只有 $s$ 个非零或近似非零分量，能否用远少于 $N$ 个线性测量恢复它？
...
在相应 RIP 条件下满足 $\|\hat x-x\|_2\le C_0\eta+C_1\sigma_s(x)_1/\sqrt{s}$，其中 $\sigma_s(x)_1=\inf_{\|z\|_0\le s}\|x-z\|_1$。这才是"稳定且近似稀疏"的精确定义 [@candes2006compressive]。

##### 考核内容

- 用 approximation error 和 estimation error 解释模型选择。

- 推线性回归正规方程，并说明不可逆时为什么需要正则化。

- 推逻辑回归损失和梯度，比较 logistic loss 与 hinge loss。

- 解释决策树为何过拟合，以及剪枝、深度限制、MDL 的作用。

- 写出 SVM hard/soft margin 目标，解释 margin 与 $\|w\|$ 的关系。

- 定义核函数和 Gram 矩阵半正定条件。

- 对压缩感知说明 $K\ll N$、稀疏性、$\ell_1$ 松弛和 RIP 的关系。

## 优化方法：从凸性到现代一阶算法

**来源定位：** 本节对应考纲"凸集和凸函数、次微分、梯度下降、随机梯度下降、随机坐标下降、动量加速、自适应学习率"。基础参考 [@nesterov2018convex; @beck2017first; @shalev2014uml]。

### 凸集、凸函数与一阶条件

集合 $C\subseteq\mathbb{R}^d$ 是凸的，表示任意 $x,y\in C$ 和 $\theta\in[0,1]$，都有

$$
\theta x+(1-\theta)y\in C.
$$

函数 $f:C\to\mathbb{R}$ 是凸的，表示
...
若函数不可微，使用次梯度。向量 $g\in\mathbb{R}^d$ 是 $f$ 在 $x$ 的次梯度，若
...
若 $A\in\mathbb{R}^{m\times d}$，$b\in\mathbb{R}^m$，则仿射复合满足
...
这些性质解释了为什么 Lasso、hinge loss 和复合优化能用"光滑梯度 + 非光滑次梯度/prox"的形式处理 [@nesterov2018convex; @beck2017first]。
...
强凸性至多说明*若*极小点存在则它唯一；存在性还可由例如闭且有界的可行域上的下半连续性，或无约束情形中 $f$ proper、lower-semicontinuous 且 coercive 保证。若 $f$ 同时 $L$-smooth、$\mu$-strongly convex，且极小点 $x^*$ 存在，梯度下降取 $\eta=1/L$ 时有
...
假设 $f$ 凸，$w^*\in\operatorname*{arg\,min}_w f(w)$，并且随机梯度满足
...
考试中的 prox-SGD 递推经常不是只用 prox 的定义，而是用它的最优性条件和非扩张性。先设 $R:\mathbb{R}^d\to(-\infty,+\infty]$ 是 proper closed convex 函数，步长 $\gamma>0$，并定义
...
这里 $g_k$ 是第 $k$ 步的梯度或随机梯度估计。后续再展开平方、取条件期望，就进入 SGD 方差分解和强凸收敛分析 [@beck2017first; @nesterov2018convex]。
...
- 用 smoothness 推梯度下降下降量。

- 比较 GD、SGD、mini-batch SGD 和随机坐标下降的计算代价与方差来源。

- 写出 prox 的定义，并推 $\ell_1$ prox 的 soft-thresholding 形式。

- 解释动量、Nesterov、AdaGrad/Adam 的状态变量和适用场景。

## 深度学习：网络、训练、泛化与生成模型

**来源定位：** 本节对应考纲"深度学习基础、视觉任务、无监督学习、视觉生成模型、表达力分析"。基础神经网络参考 [@goodfellow2016deep; @bishop2023deep; @jurafsky2026slp]；视觉任务参考 [@szeliski2022vision]；生成模型参考 [@goodfellow2014gan; @kingma2014vae; @dinh2017realnvp; @ho2020ddpm; @song2021score]。
...
这里 $h_\ell\in\mathbb{R}^{d_\ell}$ 是第 $\ell$ 层激活向量，$a_\ell\in\mathbb{R}^{d_\ell}$ 是激活前向量，$W_\ell\in\mathbb{R}^{d_\ell\times d_{\ell-1}}$，$b_\ell\in\mathbb{R}^{d_\ell}$，非线性函数 $\phi$ 按坐标作用。考试推导时先写清这些维度，可以避免把 $W_\ell^\top$ 和 $W_\ell$ 的方向写反。

### 反向传播

反向传播只是链式法则的动态规划。设单样本损失为 $J=\mathcal{L}(h_L,y)$，定义
...
经典 universal approximation 结论可以概括为：若 $K\subset\mathbb{R}^d$ 是 compact set，$f:K\to\mathbb{R}$ 连续，那么在适当非线性激活下，对任意 $\varepsilon>0$，存在足够宽的神经网络 $F_\theta$，使得

$$
\sup_{x\in K}|F_\theta(x)-f(x)|<\varepsilon.
$$

这里的 $K$ 是输入被限制的紧集，$\theta$ 是网络参数，$\varepsilon$ 是允许的 uniform approximation error。这条定理说明网络类表达能力很强，但它不说明 SGD 一定能找到这组参数，也不说明有限样本训练出的模型一定泛化 [@goodfellow2016deep; @bishop2023deep]。
...
其中 $\eta_t>0$ 是学习率，$\widehat{\nabla}\mathcal{L}(\theta_t)$ 是由 mini-batch、动量、归一化或自适应学习率修正后的梯度估计。考试里如果问"训练方法"，不要只写 backprop；backprop 计算梯度，SGD/Adam 这类优化器决定如何用梯度更新参数。
...
- 特征提取：把图像映射为向量表示，用于检索、分类或下游任务。

- 图像恢复：去噪、超分辨率、去模糊，本质是从退化观测恢复干净图像。

- 三维重构：从多视角、深度或隐式表示恢复几何结构。

- 光流估计：估计相邻帧中像素或特征点的运动场。

- 识别和分割：分类给整图标签，检测给框，语义分割给每像素类别，实例分割区分对象实例。
...
| 光流估计 | 像素运动场 $u(x)\in\mathbb{R}^2$ | 端点误差、亮度一致性、平滑正则 |
...
因为 $\mathbb{E}_{p_{\mathrm{data}}}\log p_{\mathrm{data}}(x)$ 与 $\theta$ 无关。
...
- 写出 MLP 前向传播和反向传播递推，解释 vanishing gradient。

- 比较 batch normalization、layer normalization 和 residual connection 的作用。

- 说明表达力、优化、泛化三者不是同一个问题。

- 按输出结构区分视觉任务。

- 写出 autoencoder、contrastive learning、VAE、GAN、flow、diffusion 的核心目标。

- 比较四类生成模型：VAE 有 ELBO，GAN 有对抗目标，flow 有显式可逆 likelihood，diffusion 学去噪或 score。

## 强化学习与偏好对齐

**来源定位：** 本节对应考纲"贝尔曼方程、Q-learning、Policy gradient"。基础 RL 参考 [@mohri2018foundations; @sutton2018rl]；LLM 偏好对齐参考 [@jurafsky2026slp; @rafailov2023dpo]。
...
对这个概率取负对数似然，就是前面的 DPO loss [@rafailov2023dpo; @jurafsky2026slp]。

##### 考核内容

- 定义 MDP、policy、return、$V^\pi$、$Q^\pi$。

- 从 return 递归推 Bellman expectation equation 和 optimality equation。

- 写出 Q-learning 更新式并解释 TD error。

- 用 log-derivative trick 推 policy gradient。

- 证明 baseline 不改变 policy gradient 期望。

- 解释 RLHF、KL 正则和 DPO 的数学目标及局限。

## 自然语言处理与大模型理论

**来源定位：** 本节对应考纲"统计建模、Word2vec、self-attention、LLM 预训练对齐、Scaling Law、基于 Embedding 的知识库推理"。主要参考 [@jurafsky2026slp; @goldberg2017nnnlp]；Transformer、Scaling Law、RAG、DPO 等补充参考 [@vaswani2017attention; @kaplan2020scaling; @hoffmann2022training; @lewis2020rag; @rafailov2023dpo]。
...
在 skip-gram 公式中，$v_w\in\mathbb{R}^d$ 是中心词 $w$ 的输入向量，$u_c\in\mathbb{R}^d$ 是上下文词 $c$ 的输出向量，$P_n$ 是负采样分布，$k$ 是每个正样本配多少个负样本。考试如果要求解释 negative sampling，应说明它把多分类 softmax 近似成"真实共现对 vs. 噪声共现对"的二分类问题，从而避免每次更新遍历整个词表 $\mathcal{V}$。
...
##### Attention 维度和缩放因子的推导

设 $W_Q,W_K\in\mathbb{R}^{d\times d_k}$，$W_V\in\mathbb{R}^{d\times d_v}$。则
...
因此未缩放的 logits 会随 $d_k$ 变大，softmax 进入饱和区后梯度变小。除以 $\sqrt{d_k}$ 后，点积方差回到常数量级，这是 scaled dot-product attention 的动机 [@vaswani2017attention; @jurafsky2026slp]。
...
- 从链式法则写出语言模型目标，解释 $n$-gram 的 Markov 假设与 smoothing。

- 推 Word2vec skip-gram 或 negative sampling 目标，解释 embedding 的语义来源。

- 写出 self-attention 的 $Q,K,V$ 公式、维度、mask 和复杂度。

- 解释 next-token pretraining、SFT、RLHF、DPO 的目标差异。

- 说明 scaling law 的经验形式和 compute-optimal trade-off。

- 比较知识库 embedding、RAG 和 MLN 的能力边界。

## 第三章自测：定义、条件与反例

以下问题用于检查是否真正掌握了"对象--假设--结论"链条。每题应先写定义，再列出所需条件，最后给出一行反例或失败模式；这样写出的答案也更接近资格考试的评分点。

1.  **泛化与模型选择。** 为什么有限类的 union bound 不能直接替代 VC 证明？请写出 ghost sample 的作用；再区分类内最优风险、近似误差和近似 ERM 的优化误差。

2.  **经典模型。** 在线性回归中何时正规方程不可逆？岭回归怎样改变该问题？给出一个在高维未缩放特征下 $k$-NN 距离失真的例子。

3.  **核与压缩感知。** 为什么"某个 Gram 矩阵半正定"不足以定义核？$2s$-RIP 保证的唯一性是什么，为什么它仍不足以单独证明 $\ell_1$ 恢复？

4.  **优化。** 强凸性与极小点存在性分别提供什么？请比较 GD、SGD 和随机坐标下降的随机来源，并列出 Adam 的一阶矩、二阶矩、偏差修正与数值稳定项。

5.  **深度学习。** 万能逼近定理没有保证哪两件事？任选视觉任务，写出其输入、输出、训练损失与一个常见失效模式。

6.  **强化学习。** Bellman 最优算子在哪个范数下是压缩映射？表格型 Q-learning 的收敛需要哪些访问、步长与噪声条件？

7.  **NLP/LLM。** 写出 attention logits、causal mask 与 padding mask 的区别；再说明预训练、SFT、偏好优化分别改变什么目标，以及 RAG 不能自动消除的失败模式。
...
**考点。** 第三章的复习顺序建议是：先掌握风险与泛化，再掌握凸优化；然后学经典模型和深度网络；最后学 RL 与 NLP。原因是后两者的大多数公式都在复用前两部分的概率、优化和函数逼近语言。

**考试提醒。** 做真题时可以按以下模板组织答案：

1.  写清对象：空间、随机变量、参数、损失或值函数。

2.  写目标：经验风险、期望回报、似然、ELBO、Bellman residual 或对齐目标。

3.  写关键性质：凸性、无偏性、集中界、递归关系、可逆性、mask 结构。

4.  写结论：样本复杂度、梯度、更新式、泛化解释或模型局限。

5.  若是开放题，补工程风险：数据泄漏、分布漂移、检索失败、优化不稳定、计算成本。

[^1]: 预测规则就是一个函数 $h:\mathcal{X}\to\mathcal{Y}$：看到新输入 $x$ 时，输出 $h(x)$。形式上，先设一对随机变量 $(X,Y)\sim\mathcal{D}$，其中 $X$ 是输入、$Y$ 是答案；"训练集由 $\mathcal{D}$ 独立同分布抽样"是指每个 $(x_i,y_i)$ 都按同一个联合分布 $P_{XY}$ 生成，且不同样本彼此独立。二分类中，$x$ 可为邮件的词频和发件人特征，$h(x)\in\{\text{垃圾},\text{正常}\}$；回归中，$x$ 可为房屋面积和位置，$h(x)\in\mathbb{R}$ 预测房价；概率分类中，模型先输出 $p_\theta(y\mid x)$，再以 $\operatorname*{arg\,max}_y p_\theta(y\mid x)$ 作为类别预测。样本只是未知总体分布的一小批随机观测，因此训练误差小并不自动说明新样本上的误差也小。
