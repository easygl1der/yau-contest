...
求真书院 2026 年 AI 方向博士生资格考试大纲列出四门核心课程：Machine Learning Theory、Advanced Deep Learning、Optimization Methods for AI、Natural Language Understanding。这四门课也是三套公开试卷的四个 Section。以下按官方大纲的条目顺序给出复习地图；每项的定义、推导和题型训练在第 [[02-primer|从零开始的理论预备]] 章及各年份真题章中展开 [@qzc-ai-syllabus-2026]。
...
2.  VC 维、Sauer 引理，以及样本复杂度与 VC 维的关系；
...
考纲指定教材为 [@shalev2014uml] 和 [@mohri2018foundations]；两条引用均链接至文末的标准书目信息与原书官网。

机器学习理论卷最稳定的考法不是背模型名称，而是证明学习保证。

必会对象如下：

- 经验风险 $L_S(h)$ 与真实风险 $L_{\mathcal{D},f}(h)$；

- VC 维、增长函数 $\tau_{\mathcal{H}}(m)$、Sauer 引理；

- 可实现 PAC 与不可实现/agnostic PAC；

- 模型选择、偏差--方差、线性/逻辑回归、树模型与 $k$-NN；

- 核函数的正定性、Hilbert 空间特征映射、SVM margin 与 hinge loss；

- 稀疏测量、$\ell_1$ 松弛与压缩感知的恢复条件。
...
1.  神经网络框架、训练方法、逼近与泛化分析；

2.  视觉任务中的特征提取、图像恢复、三维重构、光流估计、识别和分割；

3.  无监督学习中的自编码器、对比学习、隐式正则化、迁移学习和元学习；

4.  视觉生成模型中的 GAN、VAE、扩散模型、流模型及表达力分析；

5.  强化学习中的贝尔曼方程、Q-learning 与 policy gradient。

指定教材依次包括 [@goodfellow2016deep; @bishop2023deep; @bishop2006prml; @szeliski2022vision]。其中视觉任务应按输入、输出和评价指标组织，而生成模型与强化学习应能写出目标函数或递推式。

三套卷的重点可按上述条目定位：

- MLP、CNN、Transformer 的结构、参数量、反向传播、归一化与泛化；

- 表征学习、对比学习、预训练--微调与迁移/元学习的目标；

- likelihood、normalizing flow、VAE ELBO、GAN、DDPM ELBO 与 score matching；

- Bellman equation、policy iteration、actor-critic 与 policy gradient theorem。
...
3.  梯度下降、随机梯度下降与随机坐标下降；

4.  动量加速与自适应学习率。

指定教材为 [@nesterov2018convex; @beck2017first]。公开卷中常把这些对象组合为 composite optimization：
...
1.  自然语言处理任务的统计建模假设与方法；

2.  Word2vec 的数学原理；

3.  Transformer 中 self-attention 的原理与实现；

4.  大模型预训练对齐方法的原理、局限与改进；

5.  Scaling Law 的基本原理；

6.  基于 embedding 的知识库推理方法、局限与改进。

指定教材为 [@jurafsky2026slp; @goldberg2017nnnlp]。公开卷在此基础上还出现 RAG、MLN、topic model、contrastive learning、长上下文 Transformer 和 text-to-image diffusion system design；这些属于考纲主线的具体应用和延伸。

复习时把 NLP 题分成三类：

- 概念题：n-gram smoothing、representation learning、top-$p$、Transformer 优势；

- 数学题：InfoNCE 梯度、scaling law、MLN 概率、supervised LDA joint；

- 系统题：RAG、RLHF、长上下文、信息抽取全局一致性、文生图 diffusion。
