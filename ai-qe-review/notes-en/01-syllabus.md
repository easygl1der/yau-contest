# Examination Syllabus and Topic Map

Qiuzhen College's 2026 AI PhD Qualifying Examination syllabus specifies four core subjects: Machine Learning Theory, Advanced Deep Learning, Optimization Methods for AI, and Natural Language Understanding. They are also the four sections represented in the three public examination papers. The map below follows the official syllabus; definitions, derivations, and problem-type practice appear in [[02-primer|the theoretical foundations]] and the yearly worked-paper chapters [@qzc-ai-syllabus-2026].

## Machine Learning Theory

The official syllabus requires covering eight groups of topics: 

1. Supervised learning and PAC learning theory; 

2. VC dimension, Sauer's lemma, and the relation between VC dimension and sample complexity;

3. Model selection and bias-variance trade-off; 
...
The prescribed textbooks are [@shalev2014uml] and [@mohri2018foundations]. Their full bibliographic records and official publisher links appear in the reference list.

The reliable way to prepare for this section is to prove learning guarantees rather than memorize model names. Candidates should command:

- empirical risk $L_S(h)$ and population risk $L_{\mathcal{D},f}(h)$; 

- VC dimension, growth function $\tau_{\mathcal{H}}(m)$, Sauer’s lemma; 

- Realizable PAC versus agnostic PAC; 

- model selection, bias--variance, linear/logistic regression, tree models and $k$-NN; 

- positive definiteness of kernel functions, Hilbert space feature mapping, SVM margin and hinge loss; 

- sparse measurements, conditions for $\ell_1$ relaxation, and compressed-sensing recovery guarantees.
...
[@mohri2018foundations] provides a more theoretical and algorithmic supplementary perspective; for Rademacher complexity, margin, kernel and online learning, it is more suitable than the previous book as a source of review for proof details. 
...
The official outline divides this part into five groups: 

1. Neural network framework, training methods, approximation and generalization analysis; 

2. Feature extraction, image restoration, three-dimensional reconstruction, optical flow estimation, recognition and segmentation in visual tasks; 

3. Autoencoders, contrastive learning, implicit regularization, transfer learning and meta-learning in unsupervised learning; 

4. GAN, VAE, diffusion model, flow model and expressiveness analysis in visual generative models; 

5. Bellman equation, Q-learning and policy gradient in reinforcement learning. 

The designated teaching materials include [@goodfellow2016deep; @bishop2023deep; @bishop2006prml; @szeliski2022vision] in order. Among them, visual tasks should be organized according to input, output and evaluation indicators, while generative models and reinforcement learning should be able to write objective functions or recursive formulas. 

The focus of the three sets of volumes can be positioned according to the above items: 

- MLP, CNN, Transformer structure, parameter amount, backpropagation, normalization and generalization; 

- Representation learning, contrastive learning, pre-training - the goals of fine-tuning and transfer/meta-learning; 

- likelihood, normalizing flow, VAE ELBO, GAN, DDPM ELBO and score matching; 

- Bellman equation, policy iteration, actor-critic and policy gradient theorem. 
...
[@szeliski2022vision] is a structured catalog of vision tasks: each type of task can be restated in terms of input, geometric/statistical assumptions, output, and evaluation metrics. Currently, this library retains the author's official access entrance; the catalogs in the table are merged according to the official catalog of this version. 
...
| 3D and motion | Multi-view geometry, structure from motion, dense motion/optical flow. | Three-dimensional reconstruction, optical flow estimation. |
...
The official syllabus requires four sets of content: 

1. Convex sets and convex functions; 

2. Subdifferentials and their properties, and the important properties of convex functions; 

3. Gradient descent, stochastic gradient descent and stochastic coordinate descent; 

4. Momentum acceleration and adaptive learning rate. 

The designated textbook is [@nesterov2018convex; @beck2017first]. These objects are often combined into composite optimization in public volumes: 

$$
\min_{x\in \mathbb{R}^d} F(x) = f(x) + R(x),
  \qquad
  x_{k+1} = \operatorname{prox}_{\gamma R}\bigl(x_k-\gamma g(x_k,\xi_k)\bigr).
$$

 must master the following chain: 

$$
\text{strong convexity / smoothness}
  \Longrightarrow
\text{contraction of the gradient mapping}
  \Longrightarrow
\text{nonexpansiveness of the proximal map}
  \Longrightarrow
\text{one-step recursion}
  \Longrightarrow
\text{noise floor and complexity}.
$$

#### Lectures on Convex Optimization

[@nesterov2018convex] emphasizes the geometry, complexity and first-order methods of convex optimization; when connected to the past-exam questions, priority should be given to grasping the logic between smooth/strongly convex inequalities, oracle models and convergence rates. 
...
## The six contents of the official syllabus of natural language processing

 are: 

1. Statistical modeling assumptions and methods for natural language processing tasks; 

2. Mathematical principles of Word2vec; 

3. Principles and implementation of self-attention in Transformer; 

4. Principles, limitations and improvements of large model pre-training alignment methods; 

5. Basic principles of Scaling Law; 

6. Based on embedding Knowledge base reasoning methods, limitations and improvements. 

The designated textbook is [@jurafsky2026slp; @goldberg2017nnnlp]. On this basis, the open papers also include RAG, MLN, topic model, contrastive learning, long context Transformer and text-to-image diffusion system design; these are specific applications and extensions of the main line of the syllabus. 

Divide NLP questions into three categories during review: 

-Conceptual questions: n-gram smoothing, representation learning, top-$p$, Transformer advantages; 

-Mathematical questions: InfoNCE gradient, scaling law, MLN probability, supervised LDA joint; 

- System questions: RAG, RLHF, long context, information extraction global consistency, Vincentian graph diffusion. 
...
[@goldberg2017nnnlp] is a compact implementation-oriented textbook on neural NLP: starting from linear classification and calculation graphs, and gradually pushing to embedding, CNN, RNN, attention and conditional generation. 
