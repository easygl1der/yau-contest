...
向量默认用小写字母，例如 \(x,w,z\)；矩阵默认用大写字母，例如 \(X,Q,K,V\)。训练集样本量用 \(m\)，特征维度用 \(d\)，序列长度或上下文长度用 \(N\)。概率分布用花体或罗马体，例如 \(\Dcal\) 和 \(p_\theta\)。经验风险用 \(L_S\)，真实风险用 \(L_{\Dcal}\)。当 \(y\) 同时可能表示单个标签和标签向量时，单个标签写作 \(y_i\)，标签向量写作 \(\mathbf{y}\)。指示函数统一写作 \(\mathbb{I}\{\cdot\}\)。
...
以至少 \(1-\delta\) 的概率成立。这里的 \(\inf_{h\in\Hcal}L_{\Dcal}(h)\) 是 approximation error，也就是模型类本身的表达能力上限。ERM 的困难变成证明统一收敛：
...
如果某个 \(m\) 点集上的所有 \(2^m\) 种二值标记都能由 \(\Hcal\) 实现，就说该点集被 \(\Hcal\) shatter。VC 维定义为

\[
  \operatorname{VCdim}(\Hcal)
  =
  \max\{m:\tau_{\Hcal}(m)=2^m\}.
\]
...
这条引理的意义是：即使 \(\Hcal\) 无限，只要 VC 维有限，在 \(m\) 个样本上它能产生的标记模式也只有多项式多，因此可以像有限假设类一样做 union bound，只是把 \(|\Hcal|\) 换成 \(\tau_{\Hcal}(m)\)。

\paragraph{从 Sauer 到 VC 泛化界}

推导的关键是“只看样本上可能出现的标记模式”。对固定 \(x_1,\ldots,x_m\)，\(\Hcal\) 在这些点上至多产生 \(\tau_{\Hcal}(m)\) 种二值向量。把有限类 union bound 中的 \(|\Hcal|\) 替换为 \(\tau_{\Hcal}(m)\)，得到
...
\[
  ...
  \right]
  \lesssim
  \tau_{\Hcal}(m)\exp(-c m\varepsilon^2),
\]
...
\[
  ...
  \sqrt{
    \frac{
      d\log(em/d)+\log(1/\delta)
    }{m}
  }.
\]

这就是 VC 泛化界的来源。考试若要求“说明样本复杂度和 VC 维的关系”，应写出这条从增长函数到对数增长函数的过渡，而不是只报最终大 \(O\)。
...
半正定。核技巧的考试重点是两点：第一，非线性边界可以变成高维空间线性分隔；第二，泛化不单由高维维数决定，margin 与正则化才是关键。
...
但这是组合优化。压缩感知的核心结果是，在随机测量或满足 RIP 的测量矩阵下，可以用凸松弛
...
这表示 \(\Phi\) 在所有稀疏子空间上近似保持距离，因此不会把两个不同的稀疏向量压到同一个测量。考纲中把压缩感知放在机器学习理论内，通常是考“稀疏性 + 凸松弛 + 随机测量 + 样本/测量复杂度”的基本思想，而不是完整调和分析。
...
令 \(u=z_1-z_2\)。两个 \(S\)-稀疏向量之差至多是 \(2S\)-稀疏，因此若 \(\Phi\) 满足 \(2S\)-RIP，且 \(\delta_{2S}<1\)，则
...
但 \(\Phi u=\Phi z_1-\Phi z_2=0\)，于是

\[
  (1-\delta_{2S})\|u\|_2^2\le 0.
\]

由于 \(1-\delta_{2S}>0\)，只能有 \(u=0\)，即 \(z_1=z_2\)。所以 RIP 至少保证了稀疏解的可识别性。更深的压缩感知定理进一步说明，在更强 RIP 条件下，\(\ell_1\) 凸松弛不仅唯一，而且等于真正稀疏解 \parencite{candes2006compressive}。
...
强凸性给唯一最优解和线性收敛。若 \(f\) 同时 \(L\)-smooth 与 \(\mu\)-strongly convex，梯度下降取 \(\eta=1/L\) 时有
...
\[
  QK^\top\in\R^{N\times N},
  \qquad
  \softmax(QK^\top/\sqrt{d_k})V\in\R^{N\times d_v}.
\]
...
保证第 \(i\) 个位置不能偷看未来 token。多头 attention 把多个 head 的输出拼接后再线性投影。Transformer block 通常包括 attention、feed-forward network、residual connection 和 layer norm。
